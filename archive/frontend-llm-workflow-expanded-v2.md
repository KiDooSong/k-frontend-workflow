...
> 버전: v2 (2026-06-12) — v1은 `archive/frontend-llm-workflow-expanded.md` 참고.
>
> v1 대비 변경점:
...
| # | 산출물 | 생성 방식 | 비고 |
|---|---|---|---|
| 01 | ESLint/경계 규칙 세트 | 코드(설정) | fetch 금지, Pressable 금지, 임의 색상 금지, 레이어 경계 |
| 02 | zod 스키마 컨벤션 + CI 게이트 | 코드(설정) | typecheck → lint → 스키마 검증 → 테스트 |
...
| # | 산출물 | 생성 방식 | 비고 |
|---|---|---|---|
| 07 | Feature Inventory | 수동 | 기능 단위 분할 (화면보다 상위) |
| 08 | Screen Inventory | 수동 | 모든 화면 작업의 시작점 |
| 09 | User Flow Map | 수동 | 화면 간 이동의 근거 |
| 10 | Navigation Map | 수동 | Route Guard + **Deep Link** 포함 |
| 11 | Route Tree | 하이브리드 | Navigation Map에서 LLM이 제안 → 사람이 confirmed |
...
```txt
Purpose / Route / Entry Points / UI Sections
State Matrix          ← v1 #12
Interaction Matrix    ← v1 #13 (+ Analytics Event 컬럼)
...
priority_rules:
  ...
  - when: { navigation_clear: false }
    prioritize: [screen-inventory, user-flow-map, navigation-map, route-tree]
...
흐름: 기계검증 → 전역규칙 → Screen Inventory → Navigation Map → Route Skeleton
...
흐름: 기계검증 → 전역규칙 → OpenAPI codegen(또는 Manifest+zod)
      ...
      → Screen Inventory → Navigation Map → ScreenSpec
...
흐름: 기계검증 → 전역규칙 → 인벤토리/네비게이션 → Token Map/Catalog/Frame Index
...
흐름: 기계검증 → catalog:gen 파이프라인 → Guidelines → Gap Register
      → 공통 컴포넌트 안정화 (+ 스토리)
      → Feature/Screen Inventory 초안 → ScreenSpec 초안 → skeleton
...
흐름: Screen Inventory → User Flow Map → Navigation Map → Route Tree
      → Route Guard / Deep Link 정책 → Route Skeleton
      → 그 다음에야 ScreenSpec

핵심: 개별 화면 구현 절대 금지. route tree가 모든 것보다 우선.
...
└─ Screen Inventory
   └─ User Flow Map
      └─ Navigation Map (Guard, Deep Link)
         └─ Route Tree
            └─ Route Skeleton
...
1층: 앱 전체 계약
  ...
  Feature/Screen Inventory, User Flow, Navigation Map, Route Tree,
  Component Catalog(생성) + Guidelines, OpenAPI/zod 스키마, 전역 정책(Auth/Error)

2층: 화면별 계약
  ScreenSpec (통합형 1파일), Figma Component Mapping
