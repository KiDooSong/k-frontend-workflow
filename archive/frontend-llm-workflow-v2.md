...
> 버전: v2 (2026-06-12) — v1은 `archive/frontend-llm-workflow.md` 참고.
> 함께 읽기: [확장판 — 산출물 카탈로그와 우선순위](frontend-llm-workflow-expanded.md), [스킬팩 설계](frontend-workflow-skillpack-concept.md)
...
```txt
- 화면 파일 fetch 직접 호출 금지
  → no-restricted-imports + eslint-plugin-boundaries로
    features/*/screens 에서 src/api 직접 import 차단

- 임의 색상/스타일 금지 (text-[#333], p-[13px] 등)
  → eslint-plugin-tailwindcss 의 arbitrary value 금지 룰
  → tailwind config 에서 palette/spacing 을 토큰으로 제한

- Pressable/Text/View 로 버튼·카드 직접 제작 금지
  → no-restricted-syntax 로 screens/features 내 Pressable 직접 사용 차단
    (components/ui 내부에서만 허용)

- 레이어 위반 금지 (screen → api 직접 의존 등)
...
- 컴포넌트 카탈로그: docs/design-system/component-catalog.md (자동 생성)
...
|---|---|---|---|---|---|
| 쿠폰 사용 | useCoupon (candidate) | no | ['coupons'], ['coupons', id] | 토스트 + 상태 갱신 | 에러 토스트 + 롤백 |
...
## 3. 여러 화면이면 Screen Inventory와 Navigation Map을 먼저

개별 화면부터 구현하면 라우팅이 틀어진다. 전체 화면 목록과 이동 구조를 먼저 잡는다.
...
|---|---|---|---|---|---|
| AUTH-001 | 로그인 | /(auth)/login | auth | 기획 p.3 | spec ready |
...
## Auth Flow
/(auth)/login → /(tabs)/home on success

## Main Tabs
/(tabs)/home, /(tabs)/coupons, /(tabs)/my

## Stack
/coupons/[id], /notices/[id], /settings/profile
...
Expo Router는 파일 구조가 곧 라우트이므로 이 단계가 특히 중요하다.
LLM 작업 순서:

```md
1) ScreenSpec들을 읽고 Expo Router 기준 route tree를 설계해줘.
   화면 UI는 구현하지 말고 src/app 파일 구조와 _layout 구조만 제안해줘.

2) 제안한 route tree대로 src/app 하위에 빈 placeholder만 만들어줘.
...
확정되는 즉시 OpenAPI(또는 zod 스키마 코드)로 옮기고 manifest에서는 confirmed 표시만 남긴다.
...
### Response (zod 스키마로 정의, 타입은 z.infer로 파생)
```ts
...
export function useCoupons(status: CouponStatus): UseCouponsResult {
  return {
    data: couponFixtures.filter(c => c.status === status),
    isLoading: false,
    isError: false,
    refetch: noop,
  };
...
mutation hook에는 ScreenSpec의 Mutation Matrix에 적힌 **invalidate 대상 queryKey**를 그대로 반영한다.
...
- Guidelines: docs/design-system/component-guidelines.md
...
| State Matrix 커버리지 | Storybook 스토리 + 컴포넌트 테스트 | 상태 행 = 스토리 1개 |
...
```txt
 ...
 4. 와이어프레임 → ScreenSpec 변환 (화면당 1파일)
 5. Screen Inventory + Navigation Map (+ Deep Link, Route Guard)
 6. Expo Router route skeleton 생성
 7. OpenAPI 확인 → 있으면 codegen / 없으면 API Manifest(zod 스키마 포함)
 8. api client + fake hook (인터페이스 고정)
 9. fixture를 zod 스키마에서 생성
10. fake hook 기반 presentational screen 구현 (전 상태)
11. State Matrix → Storybook 스토리/테스트
12. 최종 Figma → Component Mapping → UI 보정
13. hook 내부 교체 (화면 수정 없음)
14. 통합 검증 + LLM 리뷰 (의미적 항목만)
15. golden example 확정 → 다음 화면 복제
```
