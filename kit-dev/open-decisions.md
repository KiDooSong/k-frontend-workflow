...
(`docs/frontend-workflow/global/open-decisions.md`)에 canonical 행 하나를 두고 각 ScreenSpec 또는
`shared-surface-spec`이 `decision_refs`로 참조한다. Surface는 local Open Decisions 표를 소유하지 않고,
open/malformed ref는 모든 member screen에 `source`+`via` provenance로 fan-out한다. 상세 consumer 계약은
...
global register 행 하나 + 각 ScreenSpec/shared surface의 `decision_refs`로 표현하며 다른 ScreenSpec의 local
행을 직접 참조할 수 없다. 첫 shared-surface slice에서는 한 결정이 같은 screen에 screen+surface 또는 surface+surface
여러 referrer로 도달하는 것도 허용하지 않는다.
...
ScreenSpec/shared-surface `decision_refs`를 해소해 화면별 파생값에 추가한다. referenced row는 canonical `source`를 보존하고,
...
- ✅ **교차-화면 참조** — #193에서 optional global register canonical 행 + ScreenSpec `decision_refs` + state/readiness fan-out + 검사 9 해소를 구현. #192에서 `shared-surface-spec` non-screen referrer, `via` provenance, 모든 member fan-out, duplicate-referrer fail-closed까지 같은 resolver 위에 추가했다.
