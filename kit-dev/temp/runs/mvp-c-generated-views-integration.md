...
- CHANGELOG: **단일** 신규 dated 섹션 `0.3.0-mvp-c-phase1`(중복 회피) — 생성기 landed + alias 승격 + 매니페스트 status/command 정정 + 본 run 문서 포인터. `0.3.0-mvp-c-phase1` 은 정식 태그가 아니라 MVP-C Phase 1 통합 마일스톤 라벨(이 PR 에서 확정).
...
1. **확정(이 PR)** — CHANGELOG 버전 문자열 `0.3.0-mvp-c-phase1` 은 정식 git 태그/릴리스가 아니라 MVP-C Phase 1 통합 마일스톤 라벨로 확정한다(기존 패턴 `0.1.0-mvp-a` → `0.2.0-mvp-b-rc1` 의 연속 라벨이며, 정식 릴리스 태깅은 별개 작업).
