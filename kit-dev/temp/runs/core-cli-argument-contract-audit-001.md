...
- **2026-07-12 — `adoption-probe.mjs` 행 해소** (후속 후보 ② — `--help` 실제 probe 실행·unknown/boolean/positional fail-open·explicit repo 오타 쓰기; PR #180): 공통 null-prototype `parseArgs` + `enforceCliFlagContract` allowlist 를 CLI 경계에 채택하고 side-effect-free `--help`, explicit repo existing-directory guard 를 추가했다. syntax → help → repo → semantic → normalize → run 순서를 고정해 usage/help 에서 scan/write/child process 0. 정상 no-visual/visual 결과와 draft-only·warning-first 의미, programmatic `runAdoptionProbe(flags)`, scalar last-wins, `--repo` precedence 는 무변경. 증거: [adoption-probe-cli-contract-001.md](adoption-probe-cli-contract-001.md).
- **2026-07-12 — 생성 뷰 3종 행 해소** (`route-tree`/`nav-graph`/`catalog-gen`): 공통 `parseArgs` + `enforceCliFlagContract` allowlist와 side-effect-free `--help`를 적용해 unknown/bare·빈 value/boolean=value/positional/prototype-key를 scan/write 전에 exit 2로 거부했다. `route-tree` import side effect도 `isCliEntry` guard로 제거했다. 정상 TXT/YAML/Markdown golden, custom adapter/layout, JSON/dry-run no-write, scalar duplicate last-wins, warning-first/hard-gate 상태는 무변경. 증거: [generated-views-cli-contract-001.md](generated-views-cli-contract-001.md). 기존 후속 PR #178, #179, #180 기록은 위와 같이 유지한다.
- **2026-07-12 — `telemetry.mjs` 행 해소** (일반 unknown `--outt`/`--otu`가 요청한 ledger를 쓰지 않고 정상 관측으로 내려가던 fail-open): telemetry 전용 value 16개/boolean 6개 allowlist와 raw occurrence 검증을 surface 선택/child·ingest/check read/ledger write보다 먼저 적용했다. unknown/prototype-key, bare·빈 value, boolean=value/뒤 값 흡수, positional, 뒤 duplicate에 가려진 앞선 invalid occurrence는 exit 2/no stdout/no write이며 all-valid scalar duplicate last-wins는 유지한다. 정상 surface/report/ledger/CI warning-first 계약과 package-scripts template은 무변경. 증거: [telemetry-cli-contract-001.md](telemetry-cli-contract-001.md).
- 남은 다음 우선순위: `visual-contract-bootstrap` exit 1→2 정렬 및 `readiness-eval`/`redteam` boolean=value 거부. 각각 별도 PR 로 유지한다.
