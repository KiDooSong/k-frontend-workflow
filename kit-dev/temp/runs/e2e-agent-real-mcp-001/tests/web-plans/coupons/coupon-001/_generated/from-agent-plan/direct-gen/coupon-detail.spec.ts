...
test.describe('View coupon detail', () => {
  ...
  test('View Free Shipping coupon detail', async ({ page }) => {
    await page.goto('/');
    await page.locator('[data-coupon-id="FREESHIP"]').getByTestId('view-button').click();
  ...
  test('View Save 20K coupon detail', async ({ page }) => {
    await page.goto('/');
    await page.locator('[data-coupon-id="SAVE20"]').getByTestId('view-button').click();
  ...
  test('Viewing a coupon detail clears the status region', async ({ page }) => {
    await page.goto('/');
    await page.locator('[data-coupon-id="WELCOME10"]').getByTestId('copy-button').click();
  ...
  test('Returning from detail view resets all applied coupon states', async ({ page }) => {
    await page.goto('/');
    const welcomeApply = page.locator('[data-coupon-id="WELCOME10"]').getByTestId('apply-button');
    ...
    await page.getByTestId('detail-back').click();
    await expect(page.getByTestId('coupon-list')).toBeVisible();
    await expect(page.locator('[data-coupon-id="WELCOME10"]').getByTestId('apply-button')).toHaveText('Apply');
  });
});
