...
test.describe('Apply coupon', () => {
  ...
  for (const c of cases) {
    test(`Applying ${c.title} coupon changes button label and status`, async ({ page }) => {
      await page.goto('/');
      const applyBtn = page.locator(`[data-testid="coupon-item"][data-coupon-id="${c.id}"]`).getByTestId('apply-button');
      ...
      for (const other of c.others) {
        await expect(
          page.locator(`[data-testid="coupon-item"][data-coupon-id="${other}"]`).getByTestId('apply-button'),
        ).toHaveText('Apply');
      }
    });
  }
  ...
  test('Applying multiple coupons in sequence accumulates applied states', async ({ page }) => {
    ...
    await expect(welcome).toHaveText('Applied'); // earlier applied state persists
  ...
  test('Re-applying an already-applied coupon updates the status without reverting any applied state', async ({ page }) => {
    await page.goto('/');
    const welcome = page.locator('[data-testid="coupon-item"][data-coupon-id="WELCOME10"]').getByTestId('apply-button');
    const free = page.locator('[data-testid="coupon-item"][data-coupon-id="FREESHIP"]').getByTestId('apply-button');
    ...
    await expect(page.getByTestId('status')).toHaveText('Applied WELCOME10');
    await expect(welcome).toHaveText('Applied');
    await expect(free).toHaveText('Applied'); // neither reverts to 'Apply'
  });
});
