...
| `eslint-workflow-config` (`eslint.workflow.config.mjs`) | repo-root JS | Only after path-resolution fix (§3.x, §8.2) | Deferred — no JS header convention until `lint-gen.mjs` defines its banner (§3.1) | **NOT-YET (planned)** | `status: planned`; today silently skipped by validate's docs-prefix bug |

---

# 2. Manifest contract

`artifact-manifest.yaml` is the **single source of truth** for what the guard inspects. The guard loads it the same way `validate.mjs` does — fail-hard if absent/malformed — exit 2 either way. Precise mechanism: `loadYamlOrExit` (`lib/util.mjs:174-181`) exits 2 only on a **parse error**; a **missing** file returns `null` (per the helper's own comment). So the guard must replicate validate's post-load guard — `if (!manifest || typeof manifest !== 'object' || Array.isArray(manifest)) process.exit(2)` (`validate.mjs:109-112`) — to make an absent manifest exit 2 rather than silently disable itself. Per-field reading and acting:
...
| `status` | `'active'` / `'planned'` | `active` → generator is runnable; eligible for regenerate-and-compare (§4). `planned` → **never run the generator**: it does not exist and its alias is **not** a live npm script (`workflow:catalog`/`workflow:lint-gen` live only in `package-scripts.template.json`'s `//roadmap` block, not in the kit's live `package.json`), so `npm run …` would fail with a *missing-script* error and a direct `node scripts/<gen>.mjs` would fail with `Cannot find module`. A `planned` entry whose file is *absent* is silently OK; a `planned` entry whose file is *present* and `do_not_edit:true` still gets a **header check** (catches half-finished manual stubs) — but only when its file shape has a **defined** header convention (YAML/TXT/Markdown, §3.1); a JS-shaped artifact like `eslint.workflow.config.mjs` has no convention yet, so its header check is deferred (§3.1, §8.4). No regenerate either way. |
...
- **JS-shaped artifacts (`eslint.workflow.config.mjs`):** **no header convention is defined yet.** Its generator `lint-gen.mjs` does not exist, so the emitted banner format (a `//` or `/* … */` comment) is unknown and cannot be specified from ground truth. The guard therefore **defers the header check for JS-shaped artifacts** until `lint-gen.mjs` lands and its banner is captured in Appendix A — even for a present file. The marker/header rules in this section apply only to the YAML/TXT/Markdown/in-file surfaces actually observed today.
...
A `planned` artifact with **no file** → no violation (skip). A `planned` artifact with a **file present** and `do_not_edit:true` → header **is** checked (catches half-authored stubs), per §2 — but only for the YAML/TXT/Markdown shapes whose header convention is defined in §3.1; a JS-shaped artifact (`eslint.workflow.config.mjs`) has no defined convention yet, so its header check is deferred (§3.1).
...
- **Strategy: regenerate-to-scratch-and-compare.**
- Supports `--out` as a **file path**. Invoke `node scripts/nav-graph.mjs --docs <docs> --out <scratchDir>/nav-graph.yaml`, twice. Timestamp-free and deterministic: screen ids and route keys are ordered by default `Array.sort()` (UTF-16, locale-independent — `lib/nav-graph.mjs:190,201`), while the per-node inbound/outbound **edge** arrays use `localeCompare` (`:192-193,203-205`). Compare with `normalizeGeneratedViewText` only.

## 4.5 `component-catalog.md`

- **Strategy: header-only — and only when `do_not_edit:true`; otherwise SKIP. NO regenerate (planned).**
- Today `do_not_edit:false` + `status:planned` + generator `catalog-gen.mjs` **absent**. The guard MUST NOT try to run the planned generator: `workflow:catalog` is not a live npm script (roadmap-only), and `catalog-gen.mjs` does not exist (a direct `node scripts/catalog-gen.mjs` would fail with `Cannot find module`). It performs **no** body check now. When `catalog-gen.mjs` lands and `do_not_edit` flips to `true` (and `status` to `active`), this artifact graduates to **regenerate-to-scratch-and-compare** automatically via the same data-driven rules — no new branch needed. Per followup.md §3.4, the diff/regenerate gate for catalog output **waits for `catalog-gen` to land**.
...
| `2` | Configuration / IO / parser error | Manifest **malformed** (parse error) → `loadYamlOrExit` exits 2; manifest **absent** → helper returns `null`, so the guard adds validate's post-load null/object guard (`validate.mjs:109-112`) to exit 2 (it must not rely on the helper to throw on a missing file); generator subprocess could not be spawned for an `active` entry; scratch-dir IO failure; unparseable committed file |
...
`component-catalog`, `eslint-workflow-config` (`status:planned`) have **no runnable generator**; running it errors. **Mitigation:** `status:planned` ⇒ never invoke the generator; absent file ⇒ skip entirely; present file + `do_not_edit:true` ⇒ header check only (§2, §4.7). For `eslint.workflow.config.mjs` specifically, even a present file gets **no** header check yet — no JS header convention is defined until `lint-gen.mjs` lands (§3.1). Must-not-fail on a missing generator.
