...
> [implementation-mode-policy.yaml](../../../frontend-workflow-kit/policies/implementation-mode-policy.yaml) ·
> [expo-feature.yaml](../../../frontend-workflow-kit/presets/expo-feature.yaml) ·
> [layout-profile.mjs](../../../frontend-workflow-kit/scripts/lib/layout-profile.mjs) ·
> [spec.mjs](../../../frontend-workflow-kit/scripts/lib/spec.mjs) ·
> [path-backstop.mjs](../../../frontend-workflow-kit/scripts/lib/path-backstop.mjs) ·
> [lint-policy.template.yaml](../../../frontend-workflow-kit/templates/meta/lint-policy.template.yaml) ·
...
| **merge 규칙** | `layers` 머지 | tier1 `mergeRoles` 우선순위 계승 — `preset < project-layout < domains.<d>`, **layer 단위 교체**(glob 병합 아님). | [layout-profile.mjs:70-76](../../../frontend-workflow-kit/scripts/lib/layout-profile.mjs) |
...
|---|---|---|---|---|---|---|
| 1 | **PR-0a** | catalog-gen `ui_primitive` 소비(UI_MARKER 제거) + check-gen ui/schema 바인딩 | [catalog-gen.mjs:23](../../../frontend-workflow-kit/scripts/lib/catalog-gen.mjs), `check-generated-files.mjs` | — | — | catalog-gen.test, check-gen.test 불변; 표준 ui 경로 동작 동일 |
...
| 4 | **PR-A** | `layers:` 파서 → `resolvedLayout.layers`(데이터만) | [layout-profile.mjs](../../../frontend-workflow-kit/scripts/lib/layout-profile.mjs), [expo-feature.yaml](../../../frontend-workflow-kit/presets/expo-feature.yaml) | 0a* | #5,#8,#9 | golden 불변(소비처 0); layout-profile.test 신규 케이스 |
| 5 | **PR-B** | `synthesizeModePolicy()` + 파리티 테스트/골든 (미배선) | [layout-profile.mjs](../../../frontend-workflow-kit/scripts/lib/layout-profile.mjs)+`.test` | A | #7 | forward-gate 0/14 **AND** backstop parity (§5) |
| 6 | **PR-C** | `<role>_present` 일반 fact + `fake_hook_exists` alias | [spec.mjs:308-319](../../../frontend-workflow-kit/scripts/lib/spec.mjs) | A | #9,#6 | `test:spec` green; coupon readiness 골든 불변 |
| 7 | **PR-D** | 정책 role-token 셀 = `layers.access` 생성물 + CI 새 target ★ | [implementation-mode-policy.yaml](../../../frontend-workflow-kit/policies/implementation-mode-policy.yaml), [layout-profile.mjs](../../../frontend-workflow-kit/scripts/lib/layout-profile.mjs), CI | **B**,C | #4,#7,#10 | 2면 parity + golden + 신규 멱등성 diff |
| 8 | **PR-E** | lint `layer_rules` DAG(import-경계 subset) | [lint-gen-core.mjs:54,454-456](../../../frontend-workflow-kit/scripts/lib/lint-gen-core.mjs), [lint-policy.template.yaml:15-26](../../../frontend-workflow-kit/templates/meta/lint-policy.template.yaml) | A | lint-DAG | **warning-first**; expo 3종 subset 이 현 출력 재현 |
...
- forbidden: **리터럴·blanket·`requires`·`order` 손수 셀 변경 금지**(`src/**`·`src/features/**`·`openapi.yaml`·
  `docs/**` 그대로). 재-잠금 비채택 규칙([layout-profile.mjs:211-215](../../../frontend-workflow-kit/scripts/lib/layout-profile.mjs))
...
|---|---|---|
| **catalog-gen `ui_primitive`** (PR-0a) | catalog-gen 이 `project-layout` 을 **안 읽고** `/src/components/ui/` 리터럴만 봄([catalog-gen.mjs:23](../../../frontend-workflow-kit/scripts/lib/catalog-gen.mjs)). 비표준 ui 경로 → catalog 0건 → `component_catalog_generated:false` → **rough-fixture-ui 진입 차단** → 도입 자체가 막힘. Tier3 의 `layers:` 가 의미를 가지려면 이게 먼저 열려야. | dry-run **F4** |
...
([package.json:33](../../../frontend-workflow-kit/package.json)).
