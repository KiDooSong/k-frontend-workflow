...
- 사람이 채택한 owner(policy `work_execution.owners` + owner `work_execution`)의 work unit을 `authority: scoped`로
  평가한다. 누적 phase 제한만 unit별 predicate(visual/api-contract/behavior)로 대체하고 source·decision·schema·owner·
  generated·claim 제한은 유지한다. 채택·unit·`decision_work_scopes` binding은 사람 소유 authoring이며 도구가 만들지 않는다.
...
- 소비자 문서: `docs/reference/scoped-work.md`, current-work/Stage 06·08/COMMANDS/upgrade-notes/doc-ownership 갱신.
  검증 기록: [`kit-dev/temp/runs/issue-238-d-scoped-execution-001.md`](temp/runs/issue-238-d-scoped-execution-001.md)
  (설계 W01–W40 대응: 충족 32, 충족+리뷰 2, 부분 3(W21·W23·W27), 설계상 리뷰·운영 3).
  실제 consumer pilot 채택·E 측정·버전/릴리스 변경은 포함하지 않는다.
...
- 검사 11이 모든 canonical input의 `captured_at`을 shared `isRfc3339()`로 hard 검증한다(`IP-001`).
  date-only/local datetime/invalid calendar·time·offset을 거부하고, generic producer도 caller timestamp를
  input ID 생성·mkdir·write 전에 거부한다. `--captured-at=` 및 앞선 malformed duplicate는 usage exit 2다.
...
- #202-C semantic/stale warning과 dogfood evidence는 후속이며 이 변경은 #202를 닫지 않는다.
...
- `workflow:forbidden-paths`의 API-related 판정을 project-level `isClearedAt()`에서 화면별 effective
  ...
  기존 project-level 동작과 warning-first/`--enforce` exit 계약은 유지한다. 명시적 v2 claim이 전혀
  없는 API-client path는 legacy broad 호환성으로 허용될 수 있으며 이를 회귀 테스트로 고정했다.
...
- 재리뷰 후속으로 absorbed source public ID가 중복된 absorbed/absorbed·active/absorbed 충돌의 모든 레코드에 `ambiguous-absorption-source`를 부여해 path-order redirect 선택을 막았다. visual component family 필터는 known absorbed-only만 제외하고 unknown/case-mismatched reference를 `unknown-component-family` 경고와 기존 catalog 진단에 남긴다. non-applicable packet/run Markdown은 실제 YAML null을 보존하며 `workflow:report` 직접 호출은 malformed 오류 대신 `report_applicable:false` no-report(exit 0)로 종료한다.
- 최종 재리뷰 후속으로 duplicate ambiguity 전파를 per-record lifecycle 검사 뒤로 옮기고, 명시 lifecycle field 또는 lifecycle error가 하나라도 있는 duplicate public ID 그룹 전체에 blocker를 부여한다. 따라서 malformed enum/active-with-absorbed-field와 clean record의 path 순서가 어느 쪽이든 readiness가 `docs-only`/빈 `allowed_paths`로 fail-closed하며, marker/field 없는 legacy duplicate 계약은 유지한다.
...
- validate 검사 4는 ScreenSpec inventory 후보 존재만 hard gate로 유지하고 generated/stale 가능 route-tree provenance를 입력으로 사용하지 않는다. warning-first 검사 13은 일반 route의 raw-token EXACT 계약을 유지하면서, 루트 `/`에 한해서만 기본 Expo 헤더 + 실제 `index.*` 노드로 검증된 전체 owner를 비교한다. custom adapter literal `/(app)`과 `src/app/(app).tsx`는 alias로 보지 않고, 복수 verified owner와 유일 owner의 복수 ScreenSpec 표현(`/(app)`, `/(app)/`)을 각각 후보 목록과 함께 경고한다. route-tree 부재·stale은 기존처럼 hard fail로 승격하지 않으며, standalone route-cross-check EXACT, array/plus group 제외, shared-surface·policy·CI promotion·release version은 변경하지 않았다.
...
### fix(upgrade) — encode rebased migration-note destinations

- render context에서 재배치한 migration-note destination의 generated path segment를 Markdown-safe하게 percent-encode해 공백과 `#`, `?`, 괄호 등 예약문자가 bare link 문법으로 오인되지 않게 했다. `/`와 dot segment, 원래 query/fragment는 보존하며 비정상 Unicode 인코딩 실패는 원문 유지 + deterministic unresolved로 fail-safe 처리한다.
- plan JSON과 raw migration notes, 일반 ASCII generated path bytes, 기존 payload-root escape/percent-decoded traversal/Windows cross-volume unresolved 의미는 변경하지 않았다.
- **PR review follow-up**: source destination의 정상 percent escape(`%20`, `%23`, UTF-8 sequence)를 filesystem path로 decode한 뒤 generated destination을 한 번만 canonical encode해 `%2520` 이중 인코딩을 막는다. malformed percent sequence는 원문 유지 + deterministic unresolved로 처리한다.
