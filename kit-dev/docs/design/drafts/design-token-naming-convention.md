...
head       := ns-head | role-head | <소비 레포가 선언한 확장/alias head>
ns-head    := color | space | type | radius | shadow | elevation        # 네임스페이스 직접
role-head  := <semantic 역할 — 내장 role-map 또는 dialect 로 네임스페이스에 매핑>
...
> head 생략형은 사람이 읽는 Visual Spec 에 적합하다(정본 템플릿이 이미 채택). 일반 역할(`bg`·`text`·`title`…)은 **내장 role-map**(§4.1)으로 dialect 없이 W1 분류되고, 레포 고유 역할만 dialect 로 매핑하면 된다(§4.3·§5.1 W1-NS). atomic(원시 팔레트) ID 는 §4.6.
...
| 허용 변형 | 예 | 흡수 조건 |
|---|---|---|
| head 생략(semantic·역할 우선) | `bg.surface` · `text.primary` · `background.normal` | 일반 역할은 **내장 role-map**(§4.1)으로 자동 분류; 레포 고유 역할만 dialect 로 `→ color` 등 매핑 선언 |
...
> 매핑(formalization §6 ↔ 본 문서): formalization W1 = 본 문서 W1-PARSE/W1-FORMAT/W1-NS, formalization W2 = 본 문서 W2-*. W3·W5·W6(필수 섹션·override 추적·4컬럼 유지)는 토큰 네이밍 범위 밖이라 그대로 formalization §6 / OD-VS-3 소관으로 둔다 — **단 W3 은 베이스라인(`## Component Mapping`)만 필수이고 `## Visual Spec`/`## Frame` 확장 필드는 opt-in(VS-1)이라 필수 아님**(formalization §6 W3 정정 동반 — 그래야 4컬럼-only 매핑이 경고되지 않는다). **시각 *충실도*(픽셀 일치)는 여기 없음 — OD-VS-4(비주얼 회귀).**
