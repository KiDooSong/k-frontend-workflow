...
- tier3 은 **계층 깊이(Axis 2)**만 다룬다. "같은 3계층을 어디에 두나"(Axis 1)는 [tier1-layout-profile.md](tier1-layout-profile.md), router/codegen 의미는 [tier2-router-adapter.md](tier2-router-adapter.md).
...
|---|---|---|
| **role-token allowed/forbidden 셀** (`{roles.X}`) | **`layers[].access`** (canonical, 확정) | 로더가 mode-major 정책으로 **합성**(전치). 정책 파일의 role-token 셀은 생성물화 지향 — *전환 방식/시점*만 구현 OD(§10.3 #4) |
...
- **② `layers` ↔ 정책 단일출처 / 병합 의미 — 정리됨(§5).** §5(layers 합성) vs §11(정책 불변) 충돌은 **셀 종류로
  분리**: role-token 셀은 `layers/access` authoritative(확정), 리터럴·requires·order 는 정책 손수. 머지는 tier1
  `mergeRoles` 우선순위·layer 단위 교체 계승. (authoritative 자체는 확정 — 정책 *파일*을 즉시 생성물화할지 점진
  전환할지의 *방식/시점*만 §10.3 #4. "정책 손수 유지" 같은 authoritative 역전은 선택지 아님.)
...
4. **정책 파일 전환 방식 & 머지:** authoritative=`layers/access` 는 §5 에서 확정. 정책 *파일* 처리만 택1 —
   (a) 즉시 생성물화(role-token 셀 제거 + 멱등성 CI) vs (b) 점진 전환(과도기엔 정책에 role-token 셀 잔존하되
   `layers/access` 가 single-source, CI 가 정책을 layers 에서 재생성·검증). 보수적 기본=(a). 머지는 tier1
   `mergeRoles` 우선순위·layer 단위 교체 계승.
