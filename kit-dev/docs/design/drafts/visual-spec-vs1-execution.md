...
> Status: **execution plan — 적용됨(branch `docs/visual-spec-vs1-figma-mapping`)**. 2026-06-21.
> 대상 결정: [OD-VS-1 = 옵션 (b)](visual-spec-od-decisions.md) — `## Visual Spec` 계열을 정본 템플릿에 **옵션 섹션**으로 추가(새 축 아님, 점진 적용).
> **적용 상태(이 브랜치):** 레포 오너 명시 지시로 §3.2 본문을 정본 [figma-component-mapping.template.md](../../../../frontend-workflow-kit/templates/screen/figma-component-mapping.template.md) 에 반영(`9c7d20f`)하고, `.draft.md` 를 deprecated 처리(§4 단계1)했다. policy·runtime 은 불변. **남은 것 = §4 단계2(`.draft` 제거) + main merge(PR #75) — 둘 다 사람.** 본문 일부의 "제안/미적용" 서술은 적용 전 작성분으로, 각 절 갱신 노트로 정정한다.
> 이 문서는 게이트를 신설/상향하지 않는다. confirmed 승격·OD resolve·hard gate·main merge 는 모두 사람.

---

## 0. 결론 요약

1. **VS-1 = 옵션 (b) 실행 = "정본에 옵션 섹션 추가".** 정본 [figma-component-mapping.template.md](../../../../frontend-workflow-kit/templates/screen/figma-component-mapping.template.md) 의 **4컬럼 `## Component Mapping` + `## Notes` 베이스라인은 그대로** 두고, 그 아래에 `## Provenance`·`## Visual Spec`·`## Data Corrections / Override Log`·`## Assets`·`## Gaps / Open`·`## Cross-links`(+ `## Frame` 확장 필드)를 **opt-in** 으로 더한다. `artifact_type: figma-component-mapping` 불변 → 새 frontmatter status·새 readiness fact·새 게이트가 생기지 않는다.
2. **산출물 + 적용(이 브랜치).** 산출물 (#1) 정본 patch 본문(§3) · (#2) `.draft.md` 처리(§4) · (#3) formalization 업데이트(§5) · (#4) VS-2 TODO seed([별도 파일](design-token-naming-convention.todo.md)). 초안은 "제안"으로 작성됐고, 이후 **레포 오너 명시 지시로 #1·#2 를 이 브랜치에 적용**(`9c7d20f`). [OD-VS-1 record](visual-spec-od-decisions.md) 의 "정본 편집 금지"는 *LLM 자율 편집* 가드였고 — 사람(오너) 지시·브랜치 격리·사람 merge 로 충족된다.
...
|---|---|---|
| §3.1 격리 경로 | ✅ 대체됨 — `## Cross-links` 가 "외부 수집 스크립트(런 디렉터리 — 비추적)". 단 특정 런 하드코딩 표현 잔존 | placeholder `{외부 수집 스크립트 — 소비 레포 소유, 킷 미포함}` 로 정리 |
...
| §3.4 비공개 screen id 참조 | ✅ 직접 id 없음 — 일반화됨 | "{소비 런의 worked example}" placeholder, 특정 런 표현도 제거 |

> 결론: 정본 옵션 섹션은 `.draft.md` 를 그대로 옮기지 않고, **formalization §4(교정 반영본)** 를 베이스로 §3-clean 하게 새로 적는다. 이로써 정본은 드래프트의 잔여 냄새(특정 런 표현·도구종속 provenance·"정본" facts)를 상속하지 않는다 — 차단의 *정합 의도*가 충족된다.

### 1.3 `figma_mapping_status` 사용처 — 이 제안이 건드리지 않음

- [policies/implementation-mode-policy.yaml:73-76](../../../../frontend-workflow-kit/policies/implementation-mode-policy.yaml): `final-fixture-ui.requires` 에 `figma_mapping_status >= draft`. 이 fact 는 **문서가 draft 로 존재하는가**만 본다. 옵션 섹션 신설은 이 비교를 바꾸지 않는다(섹션이 비어도 status 는 문서 frontmatter 가 결정). → **게이트 영향 0.**
...
### 3.2 적용 본문 (정본에 반영됨 — 오너 지시, 이 브랜치)

아래는 정본 `figma-component-mapping.template.md` 에 **적용한 전체 본문**이다(OD-VS-1 채택 실행 — 레포 오너 지시로 이 브랜치 `9c7d20f`). 현재 정본 파일이 이 본문과 byte-동일하다.
...
**권고: `deprecated` 1사이클 경유 후 remove (2단계). 단계1(deprecated)은 이 브랜치에 적용됨(`9c7d20f`); 단계2(remove)는 다음 정리 슬롯·사람.**
...
| 단계 | 시점 | 행동 |
|---|---|---|
| 0 | 초안 | draft 미수정(제안 단계). |
| 1 | 정본 patch(§3) 적용 직후 | ✓ **적용됨(`9c7d20f`)** — draft frontmatter `status: deprecated` + 상단 배너(→ 정본 + 본 실행안). 한 리뷰 사이클 유지. |
| 2 | 다음 정리 슬롯 (사람) | `git rm` 로 draft 제거(정본이 단일 출처). **미실행.** |
...
## 5. 산출물 #3 — `visual-spec-formalization.md` 업데이트 (적용됨, 이 브랜치)

formalization 의 stale 상태(PR69 open·정본 4컬럼)를 적용 현실에 동기화했다 — 아래 diff 를 이 브랜치 formalization 에 **적용**(+ od-decisions OD-VS-1 에 실행 노트 append; 결정 불변).
...
**(c) `## Cross-links` — 실행안 추가 + PR69 상태 최신화(머지 가정과 정합, §1.1):**
```diff
...
> 같은 맥락의 stale "open" 표기(formalization §1.2 헤더·§10·Cross-links)도 **이번 적용으로 함께 최신화**했다.

> [visual-spec-od-decisions.md](visual-spec-od-decisions.md) OD-VS-1 에 **실행 업데이트 노트를 append** 했다(결정 불변).
...
아래는 [OD-VS-3](visual-spec-od-decisions.md) 가 수용한 warning-first 후보 *부분집합*([formalization §6](visual-spec-formalization.md) 의 W1·W3·W4·W5·W6; W2 는 VS-2 후)이다. **이 세션은 어느 것도 구현하지 않는다.** 정본 템플릿에도 넣지 않는다(템플릿은 화면별로 복제되므로 로드맵 주석을 싣지 않는다). VS-3 슬롯 + *명시 지시* 가 있을 때만 `continue-on-error`(warning-only)로 도입. **hard gate 아님**, 어떤 신호도 `figma_mapping_status` readiness fact 에 합치지 않는다. (formalization §6 의 **W7=file_key 표기 soft 검사는 OD-VS-3 수용 범위 밖** — §10 Q5 의 open 으로 남는다; 누락이 아님을 표 마지막 행에 명시.)
...
| W2 | 토큰 ID **존재**(토큰 source 에 정의됨) | future / not implemented — **VS-2 후**(토큰 네이밍 규약·소비 레포 토큰 source 전제) |
| W7 | file_key 표기(REST 재호출 의도 시 `## Frame` 에 file_key placeholder) | **OD-VS-3 범위 밖** · open(§10 Q5) · soft 권고 · not implemented |
...
- **실제 비공개 file_key·카피·baseline·implementation-facts 원본을 템플릿에 넣지 않음.** 전부 placeholder.
- **policy·runtime(src/) 는 미편집.** 정본·`.draft` 는 레포 오너 명시 지시로 이 브랜치에 적용(§3·§4 단계1). confirmed 승격·OD resolve·`.draft` 제거(단계2)·main merge 는 사람.

---

## 9. 액션 상태

- ✓ §3.2 본문을 정본 `figma-component-mapping.template.md` 에 반영(옵션 (b) 옵션-섹션화) — `9c7d20f`.
- ✓ `.draft.md` → `deprecated` 배너(§4 단계1) — `9c7d20f`.
- ✓ §5 formalization 동기화 + od-decisions OD-VS-1 실행 노트 — 본 브랜치 후속 커밋.
- ☐ (사람) main merge — PR #75 게이트 유지.
- ☐ (사람, 정리 슬롯) `.draft.md` 제거(§4 단계2).
- ☐ (다음 슬롯) VS-2 — [design-token-naming-convention.todo.md](design-token-naming-convention.todo.md) 기반 네이밍 규약 드래프트.
- ☐ (병행 가능) VS-4 비주얼 회귀 evidence — hard gate 는 telemetry 후 별도 OD.
...
- PR69 드래프트(보유처): [figma-component-mapping.template.draft.md](../../../../frontend-workflow-kit/templates/screen/figma-component-mapping.template.draft.md)
