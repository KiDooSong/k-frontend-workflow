...
- **모드 사다리 정본**: [`frontend-workflow-kit/policies/implementation-mode-policy.yaml`](../../../frontend-workflow-kit/policies/implementation-mode-policy.yaml) (`order:`).
- **authoring/reconcile 계약 정본**: `global/llm-rules.md` · [`input-reconciliation.md`](../../../frontend-workflow-kit/docs/reference/input-reconciliation.md) (LLM 이 `open` 행 추가 / `resolved → open` 재오픈을 할 수 있는 단계).
