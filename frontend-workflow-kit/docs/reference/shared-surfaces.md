...
Overlap 판정에 참여하는 ScreenSpec entry는 checkout 위치와 무관한 project-relative 경로여야 한다. POSIX absolute와
Windows drive/UNC representation은 모두 `absolute-or-nonportable-path`, POSIX normalize 결과가 `..` 또는 `../`로 시작하는
relative traversal은 `invalid-path` 구조 오류로 fail-closed한다. root를 벗어나지 않는 backslash, redundant `./`/separator,
lexical dot segment는 repository-relative POSIX key로 수렴한다. 이 과정은 filesystem resolve/`realpath`나 case folding을 하지
않는다. 비교 key는 내부에서만 사용하며 state, inventory와 진단의 `entry_path`에는 저자가 작성한 raw 값을 그대로 보존한다.
