...
Overlap 판정에 참여하는 ScreenSpec entry는 checkout 위치와 무관한 project-relative 경로여야 한다. POSIX absolute,
Windows drive-absolute/drive-relative 및 UNC representation은 모두 `absolute-or-nonportable-path`, POSIX normalize 결과가
...
저자가 작성한 raw 값을 그대로 보존한다. absolute/drive 판정은 separator만 통일한 authored segments와 lexical normalization 후의
key에 모두 적용해 `./`나 dot segment로 prefix를 가리거나 `..` reduction으로 drive segment를 제거할 수 없게 한다. 같은
drive-prefix 규칙은 surface `implementation_paths`에도 적용한다.
