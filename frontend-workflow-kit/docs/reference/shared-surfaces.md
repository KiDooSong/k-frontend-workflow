...
Overlap 판정에서는 ScreenSpec entry의 raw 값을 project root 기준 absolute path로 resolve하고, root 내부 경로만
repository-relative POSIX key로 변환한다. 따라서 root 내부 absolute path와 root 밖으로 나갔다가 같은 root로 다시 들어오는
lexical traversal도 같은 key로 수렴하며, backslash, redundant `./`/separator, lexical dot segment도 비교 전에 정규화한다.
project root 외부로 resolve되는 entry는 `invalid-path`, Windows drive/UNC absolute representation은
`absolute-or-nonportable-path` 구조 오류로 fail-closed한다. 이 과정은 filesystem `realpath`나 case folding을 하지 않는다.
비교 key는 내부에서만 사용하며 state, inventory와 진단의 `entry_path`에는 저자가 작성한 raw 값을 그대로 보존한다.
