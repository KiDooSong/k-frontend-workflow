...
Overlap 판정에서는 ScreenSpec entry의 backslash를 `/`로 바꾸고 redundant `./`, separator, lexical dot segment를
repository-relative POSIX key로 정규화한다. 이 비교 key는 내부에서만 사용하며 state와 진단의 `entry_path`에는 저자가 작성한
raw 값을 그대로 보존한다.
