...
- `Target` 문법: `artifact:<artifact_id>[#<section-slug>[/<row-key>]]` 또는
  ...
  **자기 자신의** `input:<input_id>`. owner artifact 를 필수로 해 U-/INV-/VER- 의 화면별 중복 모호성을 없앤다.
  D-/C-/U-/G- row 해소는 owner 문서의 **canonical 가족 표**(`## Open Decisions`/`## Unknowns`/`## Conflicts`
  섹션·signature, conflicts/gap register 는 `artifact_type`)만 인정한다 — Notes/예시 표에 인용된 ID 로는
  해소되지 않는다. `artifact_id` 가 중복 선언된 대상은 owner 를 결정할 수 없어 해소 불가(hard)다.
...
register frontmatter 의 YAML 파싱 실패는 v1/v2 공통으로 검사 12 의 **항상-에러**다 — fm 이 빈 객체로
