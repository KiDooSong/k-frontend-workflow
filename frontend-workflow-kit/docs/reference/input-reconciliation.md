...
- fence(````` ``` `````·`~~~`)·HTML 주석 block·raw HTML block 은 **단일 state machine** 으로 섹션
  ...
  태그 줄까지 · 그 외 block-level HTML(`<div>`·`<table>`·`<details>`·닫는 태그 등)은 blank line 까지 —
  지원하지 않는 HTML container 내부를 마크다운으로 재해석하지 않는다. 이들 내부의 heading·표는
...
- INV-/VER- 토큰 존재 검사는 **visible prose** 만 본다: code(fence/indented/inline span — span 은
  ...
  reference definition · inline HTML tag/attribute · autolink** 도 제외한다(URL 이나 attribute 안의
