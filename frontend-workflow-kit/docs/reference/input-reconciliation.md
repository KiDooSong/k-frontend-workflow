...
- fence(````` ``` `````·`~~~`)·HTML 주석 block·raw HTML block 은 **단일 state machine** 으로 섹션
  ...
  태그 줄까지 · CommonMark **type 6 allowlist 태그**(`<div>`·`<table>`·`<details>` 등)와 **완결된
  단독 태그 줄**(type 7)은 blank line 까지 — 지원하지 않는 HTML container 내부를 마크다운으로
  재해석하지 않는다. autolink(`<https://…>`)·불완전 태그(`<x`)·태그 뒤에 내용이 이어지는 줄은
  block 이 아니라 문단이다(뒤의 보이는 heading/표를 삼키지 않는다). 이들 block 내부의 heading·표는
  섹션도 표도 evidence 근거도 만들지 못한다.
...
- INV-/VER- 토큰 존재 검사는 **visible prose** 만 본다: code(fence/indented/inline span — span 은
  ...
  (중첩 괄호 포함 balanced 소비) · reference-style label(`[text][INV-001]`·`![alt][INV-001]`) ·
  reference definition · inline HTML tag/attribute · autolink** 도 제외한다(URL·label·attribute 안의
  ID 는 근거가 아니다). 근거 언급은 backtick 없이 plain text(또는 link text)로 적는다.
...
입력이 **여러 화면/screen family 에 걸치면**(공통 shell/logo/header/CTA/layout ownership 이 걸린 batch visual update)
[visual-reconciliation.md](visual-reconciliation.md) 를 읽는다 — cross-screen visual contract 와
`workflow:visual-consistency`(warning-first) 가 그 문서의 소관이다. 아래 시각 vs 행동 라우팅 표는 그대로 유효하다:
cross-screen 이어도 behavior 는 여전히 ScreenSpec/Navigation Map/Open Decision 경로만 탄다.
...
| 여러 화면 공통 shell/logo/header/CTA/layout ownership | visual consistency contract (`design/visual-consistency-contract.md`) | screen file 별 ad-hoc patch 로 흩뿌리지 않는다. 계약: [visual-reconciliation.md](visual-reconciliation.md) |
