...
> `visual-consistency-contract` artifact, the `workflow:visual-consistency`
> warning-first check, and the `workflow:visual-contract-bootstrap` review-only
> adoption draft. Skills ([`visual-reconcile`](../../skills/visual-reconcile/SKILL.md),
> [`visual-contract-bootstrap`](../../skills/visual-contract-bootstrap/SKILL.md),
...
contract 가 아직 없는(또는 빈약한) consumer repo 는 수동 저작 전에
`workflow:visual-contract-bootstrap` 을 먼저 돌릴 수 있다 (스킬:
[`visual-contract-bootstrap`](../../skills/visual-contract-bootstrap/SKILL.md)).

```bash
npm run workflow:visual-contract-bootstrap -- --docs docs/frontend-workflow --src src --json
npm run workflow:visual-contract-bootstrap -- --docs docs/frontend-workflow --src src \
  --out temp/visual-contract-draft.md
```

- bootstrap 은 ScreenSpec frontmatter(domain/screen_id/route/screen_entry/status),
  figma-component-mapping coverage, component catalog, (선택 `--src`) screen_entry
  소스에서 후보를 뽑는 **review-only draft** 다. 아무 canonical 문서도 수정하지 않는다.
- family/member/owner/component/copy/exception 후보는 **사람이 확인해야 한다** —
  이름·경로·import 휴리스틱이라 오탐이 가능하고, 확신이 낮은 값은
  `needs-review`/`needs-human-review` 로 나온다. 반복 import 는 design intent 의
  proof 가 아니라 후보 증거다. 이름 정규식(shell/logo/header/CTA)에 안 잡혀도
  여러 화면이 반복 import 하는 **local** 컴포넌트는 `kind: unknown` 후보로
  표면화된다 (bare package/node_modules import 는 후보가 아니다 — ownership 은
  항상 needs-review).
- `--src` 를 지정했는데 selected ScreenSpec 에 `screen_entry` 가 하나도 없으면
  소스 휴리스틱이 통째로 돌지 못한다 — 이 경우 `skipped_checks` 에 skip 사유가
  남는다 (silent no-op 아님). `screen_entry` 를 백필하면 소스 축이 살아난다.
- **bootstrap output 은 confirmed contract 가 아니다.** draft frontmatter(`status: draft`)
  + review-only 주석으로만 나온다.
- **기존 contract 가 있으면 overwrite 하지 않는다** — existing rows 와 suggested
  additions 를 분리해 patch/proposal 로만 낸다. `--out` 이 기존 canonical contract 를
  가리키면 거부하고 `.draft.md` 경로를 제안한다 (canonical 경로 scaffold 는 파일이
  없을 때 + `status: draft` + review-only 주석일 때만).
- bootstrap 이 찾은 **catalog 에 없는 component 는 Component Gap 후보일 뿐**이다 —
  G-xxx 제안/accept 는 기존 component-gap-register 경로 그대로.
- ScreenSpec / Navigation Map / Open Decision 경로 없이 **behavior 를 확정하지
  않는다** — bootstrap 후보는 visual ownership 질문을 표면화할 뿐이다.

bootstrap 이후 권장 순서:

1. `workflow:visual-contract-bootstrap` (JSON 또는 `--out` draft)
2. draft review — 후보 family/owner/policy/gap 을 사람이 확인
3. 필요하면 Open Decision / Component Gap 생성 (resolve/accept 는 사람)
4. accepted rows 만 canonical visual-consistency-contract 에 반영
...
- contract 의 shared component 가 component catalog 에 없음
  ...
  단 해당 행의 Catalog Status 가 `domain`/`out-of-scope` 로 명시 선언돼 있으면
  ui_primitive catalog 대상이 아니므로 warning 대신 info
  (`component-catalog-out-of-scope`) 로 남긴다 — silent pass 가 아니라 명시 선언 경로다
