...
| Close or answer Unknowns | Unknown row, source/input evidence, affected ScreenSpec | Link the evidence and update status only when human-confirmed or source-backed | `workflow:validate`; readiness if the Unknown affected implementation mode | Silently close without approval/status rules |
...
| Codegen outputs | The repo's codegen command or focused `workflow:check-generated -- --artifact <id>` when configured |
