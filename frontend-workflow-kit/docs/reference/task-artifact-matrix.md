...
| Add or modify API integration | ScreenSpec API Candidates, API manifest, linked schema or contract evidence | API manifest, contract evidence (`zod`, `ts-type`, `openapi`, or `manual`), v2 Slice Paths/Tracking when partially deferred, allowed implementation paths | `workflow:state`; `workflow:readiness`; `workflow:validate`; `workflow:forbidden-paths -- --diff … --enforce`; relevant API/codegen checks if the repo provides them | Invent endpoints, DTOs, request fields, or response fields; edit a deferred slice |
...
| Resolve an Open Decision | Target ScreenSpec table or canonical [`global/open-decisions.md`](open-decisions.md), related inputs/conflicts, affected docs | Human-approved decision row/status/approval metadata; affected docs if behavior changes | `workflow:state`, readiness for every affected/referring screen, `workflow:validate`; regenerate affected views | Duplicate a cross-screen row into ScreenSpecs; leave stale conflict/unknown references when the decision supersedes them |
...
| Assess brownfield kit adoption (adoption-probe) | Probe run outputs (`adoption-report.md`, `probe-summary.json`); for visual adoption also [`visual-reconciliation.md`](visual-reconciliation.md) §Bootstrap / adoption | Nothing live — probe outputs stay under `temp/runs/adoption-probe-*` (draft/observation only) | `workflow:adoption-probe`; add `--visual` to also observe `workflow:visual-contract-bootstrap` (+ `workflow:visual-consistency` when a contract or bootstrap draft exists) against the probe scratch copy | Treat probe output as a gate or canonical artifact; apply the visual bootstrap draft to the canonical contract without human review; wire visual warnings to CI/required checks; resolve Open Decisions or accept Component Gaps from probe findings |
