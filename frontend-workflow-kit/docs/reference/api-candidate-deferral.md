...
is editable by its owning screen at `rough-fixture-ui` / `final-fixture-ui`, within
the base fixture-mode envelope (`{roles.hook}` allowed; forbidden still wins). This
lets a screen create and align its fixture fake hook at the contract location before
API integration. Everything else keeps the stricter gate:
