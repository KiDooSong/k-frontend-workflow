...
> **적용 상태(이 브랜치):** 레포 오너 명시 지시로 §3.2 본문을 정본 [figma-component-mapping.template.md](../../../templates/screen/figma-component-mapping.template.md) 에 반영(`9c7d20f`)하고, `.draft.md` 를 deprecated 처리(§4 단계1)했다. policy·runtime 은 불변. **남은 것 = §4 단계2(`.draft` 제거) + main merge(PR #75) — 둘 다 사람.** 본문 일부의 "제안/미적용" 서술은 적용 전 작성분으로, 각 절 갱신 노트로 정정한다.
