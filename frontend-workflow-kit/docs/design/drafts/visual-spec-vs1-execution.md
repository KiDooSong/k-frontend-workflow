...
+ - PR69 드래프트(보유처): `frontend-workflow-kit/templates/screen/figma-component-mapping.template.draft.md` (merged — §4 로 정식화, deprecated 예정)
...
> 같은 맥락의 stale "open" 표기가 formalization §1.2 헤더("PR69 (open, `MERGEABLE`)")·§10 본문에도 있다 — PR69 머지 확정 시 함께 최신화(사람 판단).
