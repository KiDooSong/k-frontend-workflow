...
| W3 | 필수(베이스라인) 섹션 존재 | status≥draft 일 때 베이스라인 `## Component Mapping`(+`## Frame` ref) 비어있지 않나. **`## Visual Spec`·`## Frame` 확장 필드는 opt-in(VS-1) — 필수 아님**(있을 때만 W1/W4 적용) | 존재만, 값 충실도 아님. VS-1 opt-in 정합 |
