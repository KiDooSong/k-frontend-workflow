...
### 1.2 PR69 (open, `MERGEABLE`) — 시각 계약 드래프트 이미 존재
...
- 정본 `frontend-workflow-kit/templates/screen/figma-component-mapping.template.md` 는 아직 **4컬럼만**(`Figma Frame / Node | UI 요소 | 매핑 컴포넌트 | 비고`) + `## Notes`. 값 레벨 칸 없음(= 리서치 02 §B 의 근본 원인 "담을 칸 없음").
...
두 대상 모두 **이 세션에서 즉시 적용하지 않는다**(코드/정책/템플릿 변경은 명시 지시 + OD 전제). diff 제안만 둔다.
...
| # | 후보 검사 | 신호 | 비고 |
|---|---|---|---|
| W1 | 토큰 ID 형식 | `color.*`/`space.*`/`type.*`/`radius.*`/`shadow.*` 패턴에 맞나 | 형식만. 존재 확인은 W2 |
| W2 | 토큰 ID 존재 | 참조 토큰이 토큰 source 에 정의돼 있나 | **OD VS-2(토큰 source 계약) 전제** — 그 전엔 W1 만 |
...
|---|---|---|---|---|
| **VS-1** | Visual Spec 섹션 정식 채택 | `figma-component-mapping.template.draft.md`(§3 교정본)를 정본 템플릿으로 승격할지 | §3 교정 적용 · PR69 머지 | 정본 템플릿 대체 시점 · `.draft` 제거 |
| **VS-2** | Design-token source 계약 | 토큰 ID 의 정의·검증 출처(예: Tokens Studio→Style Dictionary→NativeWind; component-catalog 생성 패턴 재사용 여부). **소비 레포 계약**으로 둘지 | VS-1(토큰 ID 칸 존재) | 토큰 source 의 소유(킷 vs 소비) · 생성기 멱등/GENERATED 마커 |
...
> **수용(2026-06-21):** VS-1~VS-4 모두 권고 옵션대로 사람-수용됨(VS-1=b · VS-2=a · VS-3=a · VS-4=a) → [결정 기록](visual-spec-od-decisions.md). 수용은 *방향 채택*이며 hard 게이트·코드/CI 구현은 미포함(향후 별도 사람 OD + 순차 슬롯). 즉시 다음 액션 = VS-1 전제 해소(PR69 머지 + §3 Patch B).
...
- **코드/정책/CI 미변경.** 이 세션 산출물은 설계 드래프트(이 문서) + cross-link 인덱싱뿐. 템플릿/policy 변경은 OD 후.

---

## 10. Open Questions

1. VS-1 채택 시 정본 `.template.md` 의 기존 4컬럼 사용처(골든 예제·기존 화면)를 한꺼번에 마이그레이션할지, `## Visual Spec` 을 *옵션 섹션*으로 두고 점진 적용할지?
2. 토큰 source(VS-2)의 소유: 킷이 토큰 ID *네이밍 규약*만 정의하고 값/생성은 소비 레포에 둘지, 아니면 component-catalog 처럼 킷이 생성 패턴까지 제공할지?
3. `## Data Corrections` 의 override 가 Figma variant 수정 요청을 동반할 때, 그 요청을 ScreenSpec Open Decision 으로 올릴지 이 문서 로컬로 둘지?
4. 모달/오버레이 다중 프레임을 한 `figma-component-mapping.md` 에 둘지(현 드래프트) vs 화면당 1프레임 원칙을 유지할지?
5. W7(file_key 표기)는 "REST 재호출 의도"를 어떻게 판별하나 — 항상 권고 vs sources.type=figma 일 때만?
