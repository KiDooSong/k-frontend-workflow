...
| **티어 3** | 아키텍처 **계층 깊이** (repository/use-case/VM 등) | **데이터 선언** (순서 있는 `layers:`) | screen→hook→api 3계층을 넘는 N계층 (depth 축) | [tier3-layer-model.md](tier3-layer-model.md) |
...
- [tier3-layer-model.md](tier3-layer-model.md) — 순서 있는 `layers:` 선언(role glob + 완성도 fact +
  진입 모드), spec.mjs fact 일반화, mode↔layer 결합, layer-boundaries N계층, expo-feature byte-동치 회귀.
