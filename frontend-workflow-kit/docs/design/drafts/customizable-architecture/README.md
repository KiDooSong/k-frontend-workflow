...
| # | 위치 | 무엇 |
|---|---|---|
| 4 | `scripts/lib/spec.mjs:308-318` | `fake_hook_exists` = `<srcDir>/features/<domain>/hooks` 존재검사 (**라이브 게이트 fact**) |
...
커스터마이즈는 메커니즘 기준 세 종류로 갈린다. 이 구분이 이 디렉토리 전체의 뼈대다.
...
> 세 티어는 **메커니즘** 구분(config vs plugin vs 데이터 선언)이지 *파일* 구분이 아니다. 소비자 커스터마이즈
> 파일은 `project-layout.yaml` **하나**이고, 거기서 `roles`/`domains`(티어1, 순수 config)·`adapters`(티어2,
> plugin 선택)·`layers`(티어3, 데이터 선언)를 **모두** 담는다. (tier2 §3 의 `adapters.router` 가 이 파일에 사는 이유.)
