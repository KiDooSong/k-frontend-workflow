...
CI 관측 decision 까지 완료됐다. PR-5 의 선택은 `no CI`/`warning-first CI`/
`hard CI` 중 **warning-first CI** 다. CI 는 `lint-gen --check` 와
`lint-baseline --json` 을 `continue-on-error` 로 실행해 로그만 남기며,
hard gate·required check·`--enforce` 승격은 별도 Open Decision 전까지 하지 않는다.
...
| lint CI gate promotion (MVP-B PR-5) | 경고-전용 CI 관측 | `workflow:lint-gen -- --check` + `workflow:lint-baseline -- --json` 을 CI 에 `continue-on-error` 로 배선. hard gate/required check/`--enforce` 승격 금지 |
...
PR-5 CI 는 위 lint 명령을 다음처럼 관측 전용으로만 실행한다:
...
- lint hard gate promotion decision — PR-5 는 warning-first CI 관측만 선택했다. hard gate, required check, `lint-baseline --enforce` CI 승격은 telemetry 와 별도 Open Decision 후속.
