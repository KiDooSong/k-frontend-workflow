...
argument — as exit 2 **before** reading state or writing any file. A typo never
silently falls back to a default run.
...
npm run workflow:doc-drift -- --include release-consistency --json
npm run workflow:doc-drift -- --include release-consistency --now 2026-07-11 --json
...
`workflow:doc-drift --include release-consistency` (issue #163) enables narrow structural release/version ↔ implemented-status contradiction checks — narrow rules only, never semantic inference over free prose, and the detector never edits any document. The rules: `package-version-changelog-mismatch` (package.json `version` vs the latest CHANGELOG release heading; a missing/unusable package version is itself an exit-0 warning, never a silent skip), `roadmap-snapshot-stale` (the roadmap's current `스냅샷: YYYY-MM-DD` date vs the latest release heading date; an explicit `--now YYYY-MM-DD` plus `--max-snapshot-age-days <n>` (default 30) adds an age rule — no wall clock is ever read, so output stays deterministic), `script-doc-unimplemented-contradiction` (an existing namespaced package script named exactly on a canonical doc line that also says 미구현/not implemented), and `fixed-count-mismatch` (fixed doc counts: `검사 N종` vs the validate.mjs success line is a warning; `스크립트/CLI N개` vs top-level `scripts/*.mjs` files is info-only because the count basis is a convention). Manifest active/planned ↔ roadmap status (issue #163 scope item 2) stays covered by the existing `--include status-heuristic`; the two includes compose (`--include status-heuristic,release-consistency`). Rules scan only a canonical doc allowlist (`README.md`, `IMPLEMENTING.md`, `frontend-workflow-kit/README.md` under `--root`, plus the roadmap) — historical/archive docs are excluded: any allowlisted doc carrying an uppercase `HISTORICAL` marker (or 🗄) in its first lines is skipped mechanically, and the roadmap's prior-snapshot (`이전 스냅샷`) lines — historical content inside a living doc — are excluded from every rule input, including snapshot-date extraction. Every finding carries `canonical_owner` (the truth-holding file) and `fix_path` (the file a human should edit). Findings are warning-first observations: default exit 0 always, never a gate, no hard-gate promotion, and fixing the drift stays a human action. `--package`/`--changelog`/`--roadmap` override the defaults; missing/corrupt explicitly requested inputs and invalid `--now`/`--max-snapshot-age-days` are usage/input errors (exit 2).
...
Default telemetry does not run the doc-drift status heuristic — the doc-drift surface keeps its Phase 0 args. `--doc-drift-include status-heuristic` is the explicit opt-in: it forwards `--include status-heuristic` to the default doc-drift surface. Whenever the child report carries an `info_count` (the status heuristic, or default-run info findings such as ambiguous bracket notation and root-escaping links), the surface preserves it alongside `warning_count`. Info findings are info-only/manual review: they never inflate `warning_count`, never change any exit code, and are not a semantic-truth claim. An unknown feature value exits 2. In a ledger, `inputs.doc_drift.include` is recorded only when the flag was used, so default ledgers keep the `{root, docs}` inputs byte shape.
