...
`inputs/` 의 5건을 `project-before/` baseline 에 reconcile 했을 때 **reconcile-input(+사람)** 이
...
그러나 결정을 `resolved` 로 닫거나, 충돌을 `closed` 로 닫거나, status 를 `confirmed` 로 승격하거나, 갭을 `accepted` 로 받는 것은 **사람만** 한다.

이 `expected-after/` 는 사람이 닫는 결정까지 마친 **이상적 end state** 다.
따라서 D-001 / D-003 / D-204 가 resolved 로, C-001 이 closed 로 보이는 것은 **사람 단계의 결과**이지 LLM 단독 출력이 아니다.
...
|---|---|---|---|
| IN-20260613-planning-001 | COUPON-001 screen-spec (UI Sections·Copy Keys), _meta/decision-log.md | D-001 에 separate-tab 근거를 입력으로 연결, status enum 충돌 소지를 Unknown 으로 올림, UI/Copy simple-update | **D-001 을 resolved(→ separate tab)로 닫음** |
...
| IN-20260613-meeting-001 | _meta/conflicts.md, app/navigation-map.md, AUTH-001 | resolved D-204 와의 충돌을 감지해 **C-001 생성**, **D-204 를 resolved → open 으로 재오픈**, returnTo 우선 규칙을 Route Guard 갱신으로 제안 | **C-001 을 closed 로, D-204 를 다시 resolved(returnTo 우선 + 기본 홈)로 닫음 (사람 수락)** |
