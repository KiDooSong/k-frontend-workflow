...
> 7개 입력을 reconcile 하기 전(project-before)과 후(expected-after)의 화면별 readiness.
...
|---|---|---|---|
| COUPON-001 | rough-fixture-ui | final-fixture-ui | IN-planning-001(D-001 resolve)·IN-api-001(D-003 resolve, U-001 resolve)·IN-figma-001(figma-mapping 생성)·IN-qa-001(State Matrix offline)·IN-testid-001(testID anchors evidence) + 사람의 status confirmed 승격 → final 도달 가능 |
...
2. `reconcile-input` 으로 inputs/ 7개를 처리 → 결과를 `expected-after/` 와 비교 (문서 델타).
