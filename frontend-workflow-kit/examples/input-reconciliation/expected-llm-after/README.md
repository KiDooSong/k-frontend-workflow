...
`inputs/` 5건을 `project-before/` baseline 에 **reconcile-input 만** 돌렸을 때(사람 단계 전) 나와야 하는 트리다.
...
| component-gap **G-001** | `open` *(동일)* | `open` — accept 는 사람 |
| **figma-component-mapping** | 생성 *(동일)* | 생성 |
| api-manifest **page envelope** | 반영 *(동일)* — 단 D-003 은 open 으로 표기 | 반영 |
| api-error-policy **offline/네트워크** | 반영 *(동일)* | 반영 |
| COUPON-001 State Matrix **offline 행** | 반영 *(동일)* | 반영 |

*(동일)* 표시는 두 트리에서 내용이 같은 산출물이다 — 전부 **순수 simple-update** 이거나 **gap 제안(open)** 이라 LLM 단독으로 완결된다.
...
## reconcile 자체는 끝났다 (register 5행 reconciled)

자식 decision(D-001/D-003/D-204)·conflict(C-001)가 `open` 이어도 **reconcile 행위는 완료**다.
`_meta/reconciliation-register.md` 의 5개 행은 모두 `Reconcile Status: reconciled` 이고,
