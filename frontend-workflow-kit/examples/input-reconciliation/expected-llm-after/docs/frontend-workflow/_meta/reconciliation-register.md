...
> 7개 입력 모두 reconcile 자체는 끝났으므로 `reconciled`. 사람 결정이 남은 입력은 `Result: pending user decision`.
...
## 메모 (LLM 단독 단계)
- **`reconciled` ≠ 자식 닫힘.** D-001/D-003/D-204/D-501/C-001/C-002 이 `open` 이어도 reconcile 행위는 끝났다. 그 차단은 readiness 다운그레이드가 담당한다(register 가 중복 차단하지 않는다).
- **`pending user decision`** 행(planning/api/meeting/policy-migration)은 "처리는 됐고 사람 결정만 남았다"를 뜻한다. `accepted` 로 바뀌는 것은 사람이 닫은 뒤(expected-after).
...
- **Tier3 migration 은 review-only.** readiness access wired 와 policy draft generated 는 기록하지만 live policy replacement / hard gate / CI promotion 은 하지 않는다.
