...
| fail-duplicate-endpoint | exit 1 (검사 8) | 같은 (Method,Path) 가 다른 contract/kind/source/confidence 로 중복 선언(canonical 모순) |
...
- **TS type evidence:** `Contract Kind=ts-type` 은 프로젝트 내부 `Source` 파일/디렉토리의 `export type Foo = ...`,
  generic `export type Foo<T> = ...`, 또는 `export interface Foo { ... }` 를 정적 contract evidence 로 인정한다.
  명시적 type re-export(`export type { Foo }`, `export { type Foo }`)만 스캔한다. TS type evidence 는 런타임 validation evidence 가 아니다.
- **OpenAPI/manual evidence:** `Contract Kind=openapi` 는 프로젝트 내부 `Source` OpenAPI 파일(.yaml/.yml/.json)에,
  `manual` 은 프로젝트 내부 `Source` 문서/스펙 파일(.md/.txt/.yaml/.yml/.json)에 Linked Contract 문자열이 있어야 confirmed 를 만족한다.
- **unknown:** 호환/추적용 값이며 confirmed API evidence 를 만족하지 않는다. 확정 전이면 ScreenSpec/API manifest confidence 를 candidate 로 둔다.
