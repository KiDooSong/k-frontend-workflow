...
`figma_mapping_status` 는 mapping artifact 의 존재/라이프사이클 상태만 뜻한다. pixel fidelity, token completeness,
visual regression green 을 의미하지 않으며 readiness hard gate 로 쓰지 않는다. visual fidelity 는 Verification evidence 로 다룬다.
