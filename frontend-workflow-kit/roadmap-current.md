...
1. **lint-pack / adapt-lint-pack — 설계 refresh 완료, PR-1 docs-only 완료** — 기존 로드맵의 `lint-gen/lint-baseline(MVP-B)` 생성물 lint 게이트와 **동일 개념**의 라벨(별도 신규 스킬/파일 아님 — `lint-gen.mjs`+`lint-baseline.mjs` 생성기 + 브라운필드 도입용 `adapt-lint-pack` 스킬, frontend-workflow-kit-implementation.md §11). 설계 refresh: [lint-pack-design-refresh.md](temp/proposals/lint-pack-design-refresh.md). PR-1 은 `docs/frontend-workflow/_meta/lint-policy.yaml` canonical path, [policy catalog](docs/workflows/lint-policy-catalog.md), [rollout/ratchet adoption](docs/workflows/lint-policy-rollout-ratchet.md), schema/template/manifest source 정렬만 완료했다. 잔여 PR-2 는 `lint-gen.mjs` skeleton + deterministic `eslint.workflow.config.mjs` emission 이며, 아직 `lint-baseline`, package scripts, CI 게이트는 없다.
