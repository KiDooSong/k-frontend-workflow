...
2. **Tier2 codegen 어댑터 — 첫 slice 완료, 실제 생성물 잔여** — route-tree 어댑터 솔기(#49)에 이어 `scripts/lib/codegen-core.mjs`(결정성/정렬/렌더/출력 naming) + `scripts/adapters/codegens/openapi-client.mjs` + codegen 매니페스트 + `examples/codegen-adapter/openapi-client` golden fixture 를 추가했다. 이번 slice 는 후보 manifest 로 byte-identical 결정성만 증명하며, `catalog/artifact-manifest.yaml` 등록·실제 TS client/hook 파일 생성·validate/readiness/nav-graph 직접 소비·CI/hard gate 승격은 하지 않는다. **잔여:** 실제 client/hook emitter, generated-file guard 정합, custom codegen adapter dogfood, OD-5/OD-6/OD-7 확정, validate/nav-graph 관계 문서화의 다음 slice. 설계: `temp/proposals/tier2-router-codegen-adapter.md` §17 PR-3. 기록: [tier2-route-tree-adapter-seam-001.md](temp/runs/tier2-route-tree-adapter-seam-001.md), [tier2-codegen-adapter-seam-001.md](temp/runs/tier2-codegen-adapter-seam-001.md).
