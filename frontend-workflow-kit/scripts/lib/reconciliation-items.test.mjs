...
test('markdown 전처리: HTML type 1/6 exact 경계와 type 7 paragraph 조건을 지킴', () => {
  // autolink 는 문단 인라인이다 — 뒤의 보이는 heading/표를 삼키면 안 된다.
  const autolink = stripNonContent(['<https://example.test>', '## Reconciliation Items', '| 표 |'].join('\n'));
  assert.ok(autolink.includes('## Reconciliation Items'));
  assert.ok(autolink.includes('| 표 |'));

  // 불완전 태그(`<x`)도 block 이 아니다.
  const incomplete = stripNonContent(['<x', '## Reconciliation Items', '| 표 |'].join('\n'));
  assert.ok(incomplete.includes('## Reconciliation Items'));

  // type 1은 space/tab/EOL/>만, type 6은 그에 더해 정확한 `/>`만 tag-name 경계로 인정한다.
  // type 7 complete-tag 문법도 만족하지 않는 source는 문단 text다.
  for (const malformed of ['<pre/x', '<div/x', '<pre/>', '<custom-tag =>']) {
    const cleaned = stripNonContent([malformed, '## Reconciliation Items', '| 표 |'].join('\n'));
    assert.ok(cleaned.includes('## Reconciliation Items'), `heading hidden after ${malformed}`);
    assert.ok(cleaned.includes('| 표 |'), `table hidden after ${malformed}`);
  }

  // block boundary 의 완결된 단독 태그 줄(type 7)과 allowlist 태그(type 6)는 blank line 까지 block.
  const type7 = stripNonContent(['<custom-tag attr="x">', '| 숨은 표 |', '', '뒤'].join('\n'));
  assert.ok(!type7.includes('| 숨은 표 |'));
  assert.ok(type7.includes('뒤'));

  // type 7은 열린 paragraph를 interrupt할 수 없다. standalone tag가 inline raw HTML로 이어진다.
  const inlineType7 = stripNonContent(
    ['설명', '<custom-tag>', '## Reconciliation Items', '| 보이는 표 |'].join('\n'),
  );
  assert.ok(inlineType7.includes('## Reconciliation Items'));
  assert.ok(inlineType7.includes('| 보이는 표 |'));

  // block boundary의 단독 `===`는 setext heading이 아니라 paragraph text다.
  const afterEquals = stripNonContent(
    ['===', '<custom-tag>', '## Reconciliation Items', '| 보이는 표 |'].join('\n'),
  );
  assert.ok(afterEquals.includes('## Reconciliation Items'));

  // lowercase `<!foo` 는 CommonMark type 4 declaration — `>` 포함 줄까지가 raw HTML 이다
  // (HTML 렌더링에서도 bogus comment 로 숨는 구간이라 non-content 가 렌더링과 일치한다).
  const decl = stripNonContent(['<!foo', '## Hidden Heading', '>', '뒤 내용'].join('\n'));
  assert.ok(!decl.includes('## Hidden Heading'));
  assert.ok(decl.includes('뒤 내용'));
});
...
test('markdown 전처리: 빈 container marker 뒤 type 7 block을 paragraph로 오인하지 않음', () => {
  for (const marker of EMPTY_CONTAINER_MARKERS) {
    ...
    assert.ok(!cleaned.includes('| IN-A | 01 |'), `hidden table visible after ${marker}`);
    assert.ok(cleaned.includes('뒤'));
  }

  // 빈 list item은 이미 열린 paragraph를 interrupt하지 못한다. boundary 보정이 이 규칙을
  // 뒤집어 type 7 block을 잘못 열지 않는지도 함께 고정한다.
  for (const marker of ['+', '*', '1.', '1)']) {
    const stillParagraph = stripNonContent(['설명', marker, '<custom-tag>', '| 보이는 표 |'].join('\n'));
    assert.ok(stillParagraph.includes('| 보이는 표 |'), `paragraph interrupted by ${marker}`);
  }
});

test('v2 hard: inline/type-boundary HTML 뒤의 중복 Items heading 은 숨겨지지 않음 (RR-SCHEMA-018)', (t) => {
  const dupAfter = (prefixLine) =>
    runV2(t, {
      registerExtra: [
        prefixLine,
        '## Reconciliation Items',
  ...
  assert.ok(hasCode(dupAfter('<https://example.test>').errors, 'RR-SCHEMA-018'));
  assert.ok(hasCode(dupAfter('<x').errors, 'RR-SCHEMA-018'));
  assert.ok(hasCode(dupAfter(['설명', '<custom-tag>'].join('\n')).errors, 'RR-SCHEMA-018'));
  assert.ok(hasCode(dupAfter('<div/x').errors, 'RR-SCHEMA-018'));
  assert.ok(hasCode(dupAfter('<pre/x').errors, 'RR-SCHEMA-018'));
  assert.ok(hasCode(dupAfter('<pre/>').errors, 'RR-SCHEMA-018'));
  assert.ok(hasCode(dupAfter('<custom-tag =>').errors, 'RR-SCHEMA-018'));
  assert.ok(hasCode(dupAfter(['===', '<custom-tag>'].join('\n')).errors, 'RR-SCHEMA-018'));
});
...
test('v2 hard: link/reference source 의 INV 토큰은 visible prose 가 아님', (t) => {
  ...
  const cases = [
    '[조사 문서][INV-001]\n\n[INV-001]: https://example.test/investigation', // full reference label
    '[조사 문서][\nINV-001\n]\n\n[INV-001]: https://example.test/investigation', // 여러 줄 reference label
    '[조사 문서](https://example.test/path(v2)/INV-001)', // 괄호 중첩 destination
    String.raw`[조사 문서](https://example.test/a\)/INV-001)`, // escaped `)`도 destination 내부
    '[조사 문서](<https://example.test/a)/INV-001>)', // angle destination 안의 `)`은 구조 문자가 아님
    '[조사 문서](\n  https://example.test/INV-001\n)', // destination 앞/뒤 line ending
    '[조사 문서](https://example.test/doc\n  "INV-001 내부 메모")', // title 구분 line ending
    ...
    "[probe]: https://example.test/investigation '\nINV-001 title\n'", // 여러 줄 title
  ];
  ...
  const pass = runV2(t, {
    files: { 'domains/coupons/screens/coupon-list/screen-spec.md': shortcut },
    itemRows: [...DEFAULT_ITEM_ROWS, invItem],
    summaryRows: summaryWithInv,
  });
  assert.deepEqual(messages(pass.errors), []);

  // source syntax이 아닌 plain text ID는 visible prose 근거다.
  const plainText = SCREEN_SPEC_DOC + '\n## Notes\n\nINV-001 조사 결과를 확인했다.\n';
  const plainPass = runV2(t, {
    files: { 'domains/coupons/screens/coupon-list/screen-spec.md': plainText },
    itemRows: [...DEFAULT_ITEM_ROWS, invItem],
    summaryRows: summaryWithInv,
  });
  assert.deepEqual(messages(plainPass.errors), []);

  // reference definition은 열린 paragraph를 interrupt하지 못하므로 이 source의 ID는 실제 visible text다.
  const paragraphDefinition = SCREEN_SPEC_DOC + '\n## Notes\n\n설명\n[INV-001]: /still-visible\n';
  const paragraphPass = runV2(t, {
    files: { 'domains/coupons/screens/coupon-list/screen-spec.md': paragraphDefinition },
    itemRows: [...DEFAULT_ITEM_ROWS, invItem],
    summaryRows: summaryWithInv,
  });
  assert.deepEqual(messages(paragraphPass.errors), []);

  // 빈 list item은 열린 paragraph를 interrupt하지 못한다. 그 뒤 definition-looking source도
  // paragraph text라 ID가 실제로 보이며 hard reference 근거가 된다.
  const paragraphEmptyItem = SCREEN_SPEC_DOC + '\n## Notes\n\n설명\n*\n[INV-001]: /still-visible\n';
  const emptyItemPass = runV2(t, {
    files: { 'domains/coupons/screens/coupon-list/screen-spec.md': paragraphEmptyItem },
    itemRows: [...DEFAULT_ITEM_ROWS, invItem],
    summaryRows: summaryWithInv,
  });
...
test('v2 pass: structured_since 이전 legacy Summary 링크는 RR-SCHEMA-020을 만들지 않음', (t) => {
  const r = runV2(t, {
    files: { 'inputs/IN-20260601-planning-001.md': LEGACY_INPUT },
    summaryRows: [
      ...DEFAULT_SUMMARY_ROWS,
      '| IN-20260601-planning-001 | [planning](planning/source.md) | legacy-note | reconciled | legacy-result | [legacy artifact](../domains/coupons/legacy.md) | [legacy decision](../global/open-decisions.md#d-001) | - |',
    ],
  });

  assert.equal(hasCode(r.errors, 'RR-SCHEMA-020'), false);
  assert.equal(hasCode(r.errors, 'RR-SCHEMA-006'), false);
  assert.equal(hasCode(r.errors, 'RR-SCHEMA-007'), false);
  assert.equal(hasCode(r.errors, 'RR-SCHEMA-008'), false);
  assert.equal(hasCode(r.warnings, 'RR-SCHEMA-102'), false);
  assert.deepEqual(messages(r.errors), []);
  assert.deepEqual(messages(r.warnings), []);
});
