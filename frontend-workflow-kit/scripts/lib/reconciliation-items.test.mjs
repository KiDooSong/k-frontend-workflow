...
test('markdown 전처리: autolink·불완전 태그는 HTML block 이 아니고 declaration 은 spec 대로 소비', () => {
  ...
  // 완결된 단독 태그 줄(type 7)과 allowlist 태그(type 6)는 여전히 blank line 까지 block 이다.
...
test('v2 hard: autolink/불완전 태그 뒤의 중복 Items heading 은 숨겨지지 않음 (RR-SCHEMA-018)', (t) => {
...
test('v2 hard: reference label·중첩 괄호 destination 의 INV 토큰은 visible prose 가 아님', (t) => {
  const invItem =
    '| IN-20260720-meeting-001 | 02 | verification-gap | investigation-needed | create-open | investigation:INV-001@COUPON-001-screen-spec | input:IN-20260720-meeting-001#extracted-facts | inherit | statement | inherit |';
  const summaryWithInv = [
    DEFAULT_SUMMARY_ROWS[0],
