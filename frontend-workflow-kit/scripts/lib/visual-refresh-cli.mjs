...
function requireString(flags, name) {
  ...
  if (typeof value !== 'string' || value.trim() === '') throw new VisualRefreshError(`--${name}에는 값이 필요함`);
  return value.trim();
}
...
function parseTuple(argv, kind) {
  ...
  const valueFlags = kind === 'backstop' ? new Set([...commonValues, 'range', 'base', 'diff']) : commonValues;
  const booleanFlags = kind === 'backstop'
    ? new Set(['h', 'help', 'json', 'staged', 'enforce'])
    : new Set(['h', 'help', 'json']);
  enforceCliFlagContract({ argv, flags, positionals, valueFlags, booleanFlags, tool: kind === 'backstop' ? 'forbidden-paths' : 'readiness' });
  const intent = requireString(flags, 'intent');
  if (intent !== VISUAL_REFRESH_INTENT) throw new VisualRefreshError(`지원하지 않는 --intent: ${intent}`);
  const screen = requireString(flags, 'screen');
  const input = requireString(flags, 'input');
  if (kind === 'backstop' && own(flags, 'diff')) {
    throw new VisualRefreshError('visual-refresh는 --diff(name/status only)를 지원하지 않음; --staged/--range/--base를 사용하세요');
  }
  const options = {
    docs: optionalString(flags, 'docs'), src: optionalString(flags, 'src'),
    policy: optionalString(flags, 'policy'), manifest: optionalString(flags, 'manifest'),
    layout: optionalString(flags, 'layout'), ci: optionalString(flags, 'ci'),
  };
  // Lexical syntax precedes Git I/O. Physical identity belongs to the actual
  // authority view (current authoring for forward, destination B for backstop).
  ...
  if (kind === 'backstop' && checkedPath === undefined) throw new VisualRefreshError('visual-refresh backstop은 --path가 필수임');
  return { flags, screen, input, checkedPath, root: optionalString(flags, 'root'), options };
}

function publicResult(result, tuple) {
  const { _context, ...publicFields } = result;
  if (publicFields.intent_authorization?.intent === VISUAL_REFRESH_INTENT) {
    const auth = publicFields.intent_authorization;
    const snapshot = auth.snapshot || {};
    const decision = publicFields.path_authorization || null;
    // Early applicability failures are valid evaluator results. The requested
    // tuple is audit context only; never synthesize an authorized path or mode.
    publicFields.visual_refresh_audit = {
      intent: VISUAL_REFRESH_INTENT,
      selected_screen: _context?.selected_screen ?? tuple?.screen ?? null,
      input_id: auth.input_id ?? tuple?.input ?? null,
      authorized_path: _context?.authorized_path ?? auth.authorized_path ?? null,
      checked_path: decision?.checked_path ?? auth.checked_path ?? tuple?.checkedPath ?? null,
...
function emit(result, flags, tuple) {
  const serializable = publicResult(result, tuple);
  const output = flags.json ? `${JSON.stringify(serializable, null, 2)}\n` : yamlStringify(serializable, { lineWidth: 0 });
  if (own(flags, 'out')) {
...
function assertCopySourceConfined(projectRoot, source) {
  ...
  if (stat.isSymbolicLink()) throw new VisualRefreshError(`forward authority view는 symlink를 읽지 않음: ${path.relative(projectRoot, source)}`);
  const real = fs.realpathSync(source);
  if (real !== realRoot && !real.startsWith(`${realRoot}${path.sep}`)) {
    throw new VisualRefreshError(`forward authority source가 --root 밖으로 이탈함: ${path.relative(projectRoot, source)}`);
  }
}
function overlayAuthorityPath(projectRoot, destinationRoot, relative) {
  ...
  assertSnapshotPath(projectRoot, relative, { label: 'forward authority source', type: sourceStat.isDirectory() ? 'directory' : 'file' });
  assertCopySourceConfined(projectRoot, source);
  fs.rmSync(destination, { recursive: true, force: true });
  fs.mkdirSync(path.dirname(destination), { recursive: true });
  fs.cpSync(source, destination, {
    recursive: true, dereference: false,
    filter(current) { assertCopySourceConfined(projectRoot, current); return true; },
  });
}

// Forward uses current authoring but never current implementation facts. Check
// resolved role/layer separation before copying and again in the authority view.
function materializeForwardAuthorityView(repository, sourceTree, options) {
  const destination = materializeGitTree({ repositoryRoot: repository.repositoryRoot, projectPrefix: repository.projectPrefix, tree: sourceTree });
  try {
    const overlays = new Set([options.docs || DEFAULTS.docs]);
    for (const key of ['policy', 'manifest', 'layout', 'ci']) if (options[key]) overlays.add(options[key]);
    for (const relative of overlays) overlayAuthorityPath(repository.projectRoot, destination.root, relative);
    return destination;
  } catch (error) { destination.cleanup(); throw error; }
}
function isCurrentRegularFile(projectRoot, relative) {
  try {
    ...
    const realRoot = fs.realpathSync(projectRoot);
    return fs.realpathSync(absolute).startsWith(`${realRoot}${path.sep}`);
  } catch { return false; }
}
function applyForwardFileIdentity(result, projectRoot) {
  ...
  if (!intent?.applicable || !authorizedPath || isCurrentRegularFile(projectRoot, authorizedPath)) return result;
  result.intent_authorization = {
    ...intent, applicable: false,
    reasons: [...(intent.reasons || []), { code: 'VR-SCREEN-009', message: 'current forward view의 authorized screen_entry가 existing regular file이 아님' }],
  };
  if (result.path_authorization) result.path_authorization = { ...result.path_authorization, allowed: false };
  return result;
}
...
export function runVisualReadinessCli(argv = process.argv.slice(2)) {
  ...
  if (!source) {
    const output = {
      intent_authorization: {
        intent: VISUAL_REFRESH_INTENT, input_id: tuple.input, applicable: false,
  ...
  try {
    ...
    process.exitCode = 0;
    return output;
  } finally { source.materialized.cleanup(); destination?.cleanup(); }
}
function authorityViolation(authority) {
  return {
    file: '(authority)', change: '?', code: 'VR-BACKSTOP-010',
...
export function runVisualForbiddenPathsCli(argv = process.argv.slice(2)) {
  ...
  const diff = resolveVisualDiffContext({
    repositoryRoot: repository.repositoryRoot, staged: tuple.flags.staged === true,
    range: optionalString(tuple.flags, 'range'), base: optionalString(tuple.flags, 'base'),
  });
  const source = materializeGitTree({ repositoryRoot: repository.repositoryRoot, projectPrefix: repository.projectPrefix, tree: diff.source_tree });
  let destination;
  try {
    ...
    if (tuple.checkedPath && tuple.checkedPath !== authority._context?.authorized_path) {
      violations.push({ file: tuple.checkedPath, change: '?', code: 'VR-BACKSTOP-011', reason: '--path does not equal stable authorized screen_entry' });
    }
    const enforced = tuple.flags.enforce === true;
    const output = {
      ok: violations.length === 0, enforced,
      ...
      violations,
      changed_records: visualChangedRecords(diff.records, repository.projectPrefix),
      diff_context: {
        source_tree: diff.source_tree, destination_tree: diff.destination_tree,
        diff_kind: diff.diff_kind, project_prefix: repository.projectPrefix,
      },
    };
    emit(output, tuple.flags, tuple);
    process.exitCode = output.ok ? 0 : enforced ? 1 : 0;
    return output;
  } finally { source.cleanup(); destination?.cleanup(); }
}
export function runVisualCliSafely(fn, toolName) {
  try { return fn(); }
    ...
    const known = error instanceof VisualRefreshError || error instanceof VisualRefreshGitError ||
      error instanceof VisualRefreshResourceError || error instanceof ArtifactPathError;
    const message = known ? error.message : `${error?.name || 'Error'}: ${error?.message || String(error)}`;
