...
- 원인: `scripts/workflow-run.mjs` 가 `layout` 플래그를 파싱하지 않고, `packetArgs` 에 `--layout` 을 넣지 않음
...
- `scripts/workflow-report.mjs` 도 `layout` 플래그를 파싱하지 않으며, `collectValidate`/`collectForbidden`/
  `collectCheckGenerated` 호출에 `--layout` 이 없다. 이들 leaf 도구(validate 검사 8/fake_hook,
