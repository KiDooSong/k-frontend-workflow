...
| validate 형식 검사 | §6·§7 | `scripts/validate.mjs` — **검사 13 추가(WARNING-ONLY)**. `interactionMatrixV2Issues`(spec.mjs 순수 함수)를 호출해 enum 위반 / `route` 행 Target 부재 / Result↔Target drift / route-tree EXACT Target 부재 / 비-route 행 라우트 토큰 / 빈 Result Type(v1 폴백)을 **경고로만** surface. 검사 4 불변(여전히 Result 셀, v1 경로). route-tree artifact 부재 시 교차검증은 skip. |
...
- **warning-first:** v2 형식 검사(검사 13)는 **경고만** — 에러 0, exit code 영향 0. `interactionMatrixV2Issues` 는 절대 throw 하지 않는다. route-tree EXACT 교차검증을 하드 fail 로 만들지 않았다.
...
추가 수동 검증(초기 dual-read 구현 당시):
- v2 픽스처 그래프 본문이 v1 basic-flow expected 와 **byte-identical**(`diff` tail +4 동일).
- v2 픽스처 docs 에 validate 실행 → `[경고 13] … Target /coupons/[id] 가 화면 inventory 에 없음 … (warning-first)` **1건, exit 0**(차단 아님). 이후 정밀화에서 inventory 약식 경고는 route-tree EXACT 교차검증으로 대체됨.
...
- **`Result Type` enum 동결:** `scripts/lib/spec.mjs` 의 `INTERACTION_V2_RESULT_TYPES` 를 `Object.freeze(['route','state','mutation','external','none'])` 단일 출처로 고정하고, enum 경고 메시지에서 "제안값/OD-2" 표현을 제거했다. 새 값 추가는 이 상수와 테스트를 함께 바꾸는 명시 PR 로만 한다.
- **route-tree EXACT cross-check:** `scripts/lib/route-core.mjs` 에 `parseRouteTreeRouteTokens(text)` 를 추가해 route-tree.txt 출력의 `route: <token>` 만 읽는다. `[id]`, `(tabs)`, `/` 는 정규화하지 않고 그대로 비교한다.
- **검사 13 연결:** `scripts/validate.mjs` 는 `docs/frontend-workflow/_meta/route-tree.txt` 가 있을 때만 토큰 집합을 읽어 `Result Type=route` 행의 `Target` 과 EXACT 비교한다. route-tree artifact 가 없으면 skip 이며, 불일치도 `[경고 13] route-tree EXACT cross-check...` 로만 출력된다.
- **inventory 약식 경고 제거:** 기존 `Target inventory` 약식 경고는 route-tree 기반 EXACT 검증으로 대체했다. 검사 4(v1 free-form Result → screen inventory), readiness, 정책, hard gate, 템플릿 강제 전환은 그대로 유지했다.
- **검증:** `npm run test:spec` → 59 pass / 0 fail. `npm test` → test-fixtures 27 fixtures(26 pass, 1 xfail) + unit 59 pass / 0 fail.

## 후속 (이 PR 범위 밖 — 설계 §11/§12 의 잔여 Open Decision)

- **OD-1:** v2 컬럼 순서/필수성은 여전히 별도 결정. v2 컬럼은 optional 유지.
