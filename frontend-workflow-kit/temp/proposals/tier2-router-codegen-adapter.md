...
**Resolved by human decision — 2026-06-21:**
- **OD-11 — Tier2 codegen/route warning-first → CI/gate 승격 (decision_id: OD-11):** **"no CI" (현 상태 유지)** 로 닫는다. Tier2 codegen/route 의 모든 warning-first 표면(`check-generated-files` codegen focused target `codegen-openapi-client` · `route-cross-check` · v1 default guard `route-tree`/`nav-graph`/`component-catalog`)을 **warning-first(exit 0) 그대로** 둔다 — 게이트(readiness/validate)·CI·`artifact-manifest.yaml` status·golden 무변경. **승격(warning-first CI smoke / hard CI / required check / `--enforce`)은 실제 도입(adoption)이 telemetry 를 만들 때까지 연기.** 근거: PR #65 증거(`temp/runs/tier2-gate-promotion-evidence-001.md`) — 도입 전엔 멀티런 smoke 이력·방향별 FP rate·소비 트리 준비도·교차환경 결정성·사람 `decision_id` 가 없어 어떤 승격도 정당화되지 않는다(Evidence 1d component-catalog placeholder 위험 포함). **CI 재분류:** CI 를 하게 되면 GitHub Actions(`.github/workflows`)가 아니라 **GitLab(`.gitlab-ci.yml`) 기준**이어야 한다(현 킷 CI 는 타깃 환경과 불일치) — 따라서 CI 는 "승격 결정"이 아니라 **도입 작업**으로 재분류한다(이 슬라이스에서 GitLab CI 도 만들지 않는다). **재오픈 트리거:** 킷이 실제 프로젝트에 도입되어 warning-first telemetry(멀티런 smoke 이력 + 방향별 FP 분류)가 생기면 그때 재검토. (lint-pack item 1·Interaction Matrix item 3 의 승격도 같은 성격 — 도입 후 telemetry 기반 재검토 — 이나, 이 결정은 OD-11 만 닫는다(비구속 노트, 상태 변경 없음).) 기록: `temp/runs/tier2-gate-promotion-decision-prep-001.md`(## Resolution). 시각화(읽기 전용, gitignore): `docs/frontend-workflow/_viz/decision-OD-11-{easy,dev}.html`.
