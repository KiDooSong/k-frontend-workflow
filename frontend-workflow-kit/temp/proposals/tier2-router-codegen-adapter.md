...
**Resolved (2026-06-20):** ScreenSpec route ↔ 어댑터 rawPath cross-check 는 **(A) 별도 warning-only 도구**(`scripts/route-cross-check.mjs` + lib + test)로 닫혔다 — ScreenSpec frontmatter `route` 집합 ↔ route-tree.txt 의 `route: <token>` 집합(어댑터 rawPath 투영)을 **EXACT 양방향**(정규화 없음) 대조해 불일치를 경고로만 낸다(**항상 exit 0**). 비교는 6a 규약 그대로 EXACT — nav-graph 의 fm.route EXACT 매칭(lib/nav-graph.mjs:107,115)과 동형이고, parseRouteTreeRouteTokens 가 [id]/(group) raw 표기를 정규화하지 않는다(route-core.mjs:171-172). validate/nav-graph/route-tree 에 결합하지 않고(§10(10b)·NG-1) 어댑터도 직접 import 하지 않는다(산출물만 읽음). 위치/엄격도 상세 → OD-4(§16). **남은 Unknown:** urlPath 를 쓰는 어댑터의 ScreenSpec 화해(Next 프로젝트가 `[id]` 대신 `:id` 를 적을 때) → 현 샘플은 expo raw 표기뿐이라 검증 불가.
...
**Resolved (2026-06-20):** OD-8 = **nav-graph 문서-only 유지(§11b 권고)**. nav-graph 생성기는 어댑터/cross-check 를 소비하지 않는다(엣지/시드는 여전히 문서). nav 라우트 drift 도 필요하면 *같은* warning-only 도구(`scripts/route-cross-check.mjs`)가 담당하고 nav-graph 생성기에는 결합하지 않는다. 단 **첫 슬라이스는 route 차원만**(ScreenSpec route ↔ route-tree.txt) 구현하고 nav 차원(navigation-map drift)은 defer(후속 슬라이스로 기록). 상세 → OD-8(§16).
...
> **랜딩(2026-06-20) — codegen 변형:** `examples/codegen-adapter/minimal-custom`(my-codegen.mjs + 코어 생성 manifest/client/hook 골든 + README)로 랜딩, `codegen-core.test.mjs` C21–C25(route-core S3/S5/S7/S4 미러)로 고정. 라우터 변형은 `examples/router-adapter/minimal-custom`. 내장 매니페스트 등록·focused guard target·artifact 엔트리 없는 순수 단위 입력. 기록: temp/runs/tier2-codegen-custom-adapter-dogfood-001.md.
...
| ScreenSpec route | rawPath==ScreenSpec route(EXACT, 현 동작), urlPath 는 별도 산출물, cross-check = 별도 warning-only 도구(OD-4 resolved 2026-06-20) | §6 |
...
| nav-graph↔어댑터 | nav-graph 는 어댑터 직접 소비 안 함 (route-tree 만 소비), cross-check = 별도 도구(OD-8 resolved 2026-06-20, route 차원; nav 차원 defer) | §11 |
...
**Resolved by human decision — 2026-06-20:**
- **OD-4 — ScreenSpec route ↔ 어댑터 rawPath cross-check 검사:** **(A) 별도 warning-only 도구**로 닫는다. `scripts/route-cross-check.mjs`(+`scripts/lib/route-cross-check.mjs`+`scripts/lib/route-cross-check.test.mjs`)가 ScreenSpec frontmatter `route` 집합 ↔ route-tree.txt 의 `route: <token>` 집합(어댑터 rawPath 투영)을 **EXACT 양방향**(정규화 없음, §6) 대조해 불일치를 경고로만 낸다(**항상 exit 0**, warning-first). validate/nav-graph/route-tree 에 결합하지 않고(§10(10b)·NG-1) 어댑터도 직접 import 하지 않는다(산출물 2개만 읽음). 위치=별도 도구, 엄격도=warning-first(게이트 아님 — readiness/validate 불변). route-tree.txt 부재/screen-spec 0건이면 조용히 skip(검사 13 동형). 기록: temp/runs/tier2-route-cross-check-001.md.
- **OD-8 — nav-graph 어댑터 cross-check:** nav-graph 는 **문서-only 유지(§11b 권고)**. nav 라우트 drift 도 필요하면 *같은* `route-cross-check` 도구가 담당하고 nav-graph 생성기에는 결합하지 않는다. 단 **첫 슬라이스는 route 차원만** 구현하고 nav 차원(navigation-map drift)은 defer(후속 슬라이스로 기록).
...
- **OD-11 — Tier2 codegen/route warning-first → CI/gate 승격 (status: open):** roadmap item 2 의 잔여(승격 결정 하나)를 사람이 resolve 할 수 있도록 공식 등재한다. **이 설계 PR/슬라이스는 막지 않는다** — 게이트(readiness/validate)·CI(`.github/workflows`)·`artifact-manifest.yaml` status·golden 무변경, 모든 Tier2 표면 warning-first 유지. 이 등재는 결정을 *준비*만 하고 **어떤 옵션도 선택/close 하지 않는다.**
  - **Decision Needed (질문):** 기존 Tier2 warning-first 표면 — codegen focused advisory check(`codegen-openapi-client`) · `route-cross-check --json` · v1 default guard(`route-tree`/`nav-graph`/`component-catalog`) — 를 CI / hard gate / required check / `--enforce` 로 승격할 것인가. 즉 **안 둘까 / 경고만 둘까 / 막을까.**
  - **Options (evidence Decision Draft 그대로, evidence status 포함):**
    1. **no CI (status quo)** — `.github/workflows` 에 Tier2 단계 없음(현 상태). *Supported:* 모든 표면이 로컬에서 결정적·exit 0; 더 나아갈 근거가 될 멀티런 run history 가 아직 없음.
    2. **warning-first CI smoke** — codegen focused check + `route-cross-check --json` + v1 default guard 를 `continue-on-error: true` 로 배선(게이팅 X, telemetry only). lint-pack PR-5 미러지만 멀티런 smoke 이력·방향별 FP rate 부족으로 **아직 미정당화** (이 슬라이스에서 하지 않음).
    3. **hard CI / required check / `--enforce`** — 비차단 제거 · required check 화 · exit-1 게이팅. **미지원** — Evidence 1d(component-catalog placeholder mismatch → 소비 트리 깨짐) + route cross-check Direction-2 FP(layout/group/secondary route) + 교차환경 결정성 미관측 + 사람 `decision_id` 필요; `--enforce` 자체가 미구현(check-generated-files.mjs:25-26, 110-111).
  - **Blocking Mode (막는 것):** roadmap item 2 **마감**(잔여 = 이 승격 결정 하나; roadmap-current.md item 2 line 114). 어댑터/codegen 구현·다른 OD·이 설계 PR 은 막지 않는다 — 막는 것은 오직 item 2 의 close.
  - **Owner:** 사람(메인테이너) — 별도 사람-승인 decision PR + `decision_id` + 명시 rationale. (OD-4/OD-8 은 cross-check 의 *도구 형태* 만 닫았고 *승격* 은 미결로 남겼다.)
  - **근거 cross-link:** `temp/runs/tier2-gate-promotion-evidence-001.md` (특히 Decision Draft · Promotion Telemetry Needed · Evidence 1d). 선례: lint-pack `temp/runs/lint-gate-promotion-evidence-001.md`. 등재/시각화 기록: `temp/runs/tier2-gate-promotion-decision-prep-001.md`. 사람 이해 보조 시각화(읽기 전용 미리보기, gitignore 대상, OD-4 산출물 미러): repo-root `docs/frontend-workflow/_viz/decision-OD-11-{easy,dev}.html`.
  - **Recommendation:** **warning-first 유지, pending** — 어떤 옵션도 선택/closed 처리하지 않는다. resolve(open→resolved)는 사람 몫이다.
...
4. **(별도 dogfood PR) — custom-adapter 픽스처:** `examples/router-adapter/minimal-custom/` (+ codegen 변형) 회귀 고정 (§13). **PR-2/PR-3 코어 랜딩 후에만.** Tier-1 custom-monorepo 선례(custom-monorepo/project-layout.yaml) 미러. **✅ codegen 변형 랜딩(2026-06-20):** `examples/codegen-adapter/minimal-custom` + `codegen-core.test.mjs` C21–C25 — 내장 매니페스트 등록·focused guard target·artifact 엔트리 없이 단위 입력으로만 (temp/runs/tier2-codegen-custom-adapter-dogfood-001.md).
...
**2026-06-20 current slice:** OD-4/OD-8 을 위 **Resolved by human decision — 2026-06-20** 블록대로 닫고, route cross-check warning-only 도구(`scripts/route-cross-check.mjs` CLI + `scripts/lib/route-cross-check.mjs` 로직 + `scripts/lib/route-cross-check.test.mjs` 테스트)를 랜딩했다. ScreenSpec frontmatter `route` 집합 ↔ route-tree.txt `route: <token>` 집합(어댑터 rawPath 투영)을 **EXACT 양방향** 대조해 불일치를 경고로만 낸다(기본 stderr 사람-읽기 + `--json` stdout, **항상 exit 0**). 입력은 어댑터가 아니라 산출물 2개(route-tree.txt + screen-spec frontmatter)이고, validate/nav-graph/route-tree 에 결합하지 않으며(§10(10b)·NG-1) 어댑터도 직접 import 하지 않는다. route-tree.txt 부재/screen-spec 0건이면 조용히 skip(검사 13 동형). **첫 슬라이스는 route 차원만** — nav 차원(navigation-map drift)·codegen output↔docs 차원·CI/required check/hard gate/`--enforce` 승격은 별도 후속으로 유지한다. 기록: temp/runs/tier2-route-cross-check-001.md.
