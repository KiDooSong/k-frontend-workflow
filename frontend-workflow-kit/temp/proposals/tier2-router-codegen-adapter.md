...
기존 Tier-2 router 설계(tier2-router-adapter.md)는 **router 의미의 펼러그인화**를 결정했다: discover-vs-core 솔기(§5·§6), 정규화 `RouteNode` + `meta` 탈출구(§4), 파일↔코드 화해(§7), 등록 매니페스트(§8), "어댑터=발견, 코어=결정성"(§6). 그러나 그 문서는 **router 만** 다룬다. 본 문서는 task 가 요구하는 **새 차원**을 baseline 위에 얹는다:
...
**본 문서가 추가하는 새 차원** (baseline 에 없음): codegen client/hook 생성(§3·§7·§8·§9), server/client 경계(§4), link/navigation API(§5), ScreenSpec route 화해(§6), validate↔어댑터(§10), nav-graph 직접소비 여부(§11), codegen 출력경로/hook 네이밍(§9), fail-closed(§14).
...
### Recommendation — **(8b): codegen 전용 등록 매니페스트 + `project-layout.yaml.adapters.codegen` config.**
...
```
- **config 위치:** Tier-1/2 가 한 파일을 쓴다는 README 결정(README.md:52-54)대로 `project-layout.yaml.adapters.codegen`. preset(expo-feature)이 default 를 묶는다 (§2, §3).
...
### Recommendation — **(9b): codegen 출력 경로는 Tier-1 role(`api_client`/`hook`)에서 파생.** 전용 `api_generated` role 분리는 **OD-5**.
...
**미결:** 출력이 whole-file 단일 파일인가 per-endpoint 파일인가 → **OD-6**(가드 표면/결정성에 영향). hook 출력이 도메인별(`{domain}` 치환)인가 전역인가 → **OD-7**. 현 킷에 codegen 산출물이 없어 전부 **검증 불가**(Unknown).
...
| codegen 출력/hook | 출력경로=Tier-1 role 파생, hook 네이밍=어댑터 conventions config 토큰 | §9 |
...
- **OD-5 — 전용 `api_generated` role:** codegen 출력 경로를 기존 `api_client`/`hook` 에 섞을지, 생성물 전용 role 로 분리할지(§9c). role 추가는 Tier-1 변경. **막는 것:** codegen 출력 경로의 가드 표면 분리.
- **OD-6 — codegen 출력 입도:** whole-file 단일 파일 vs per-endpoint 파일(§9). 가드 reproduce 표면/결정성에 영향. **막는 것:** codegen golden fixture 의 byte-exact 형태.
- **OD-7 — hook 출력 스코프:** hook 을 도메인별(`{domain}` 치환, `{roles.hook}`)로 낼지 전역으로 낼지. **막는 것:** hook 출력 경로 파생식.
...
1. **(FUTURE) PR-1 — config schema + docs only:** `project-layout.yaml` 에 `adapters.router`/`adapters.codegen` 스키마(+JSON schema)와 본 문서 확정 규약을 *문서/스키마로만* 추가. preset(expo-feature)에 `adapters` 기본값 묶기. **코드·생성기·매니페스트 동작 변경 0** (router/codegen 코어 미존재 상태 유지). OD-1/OD-10 닫기 후보.
2. **(FUTURE) PR-2 — route-tree 어댑터 솔기:** `scripts/lib/route-core.mjs` 추출(normalize/render/write/check, baseline §5·§6) + `scripts/adapters/routers/{expo-router}.mjs` + `manifest.json`. `route-tree.mjs` CLI 가 `adapters.router` 선택→`discover`→core. **회귀: `examples/route-tree/{basic-app,edge-cases}` golden byte-identical** (§12). server/client `meta.boundary` 필드 도입(expo 는 미사용→golden 불변, §4).
