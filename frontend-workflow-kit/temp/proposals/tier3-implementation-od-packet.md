...
> resolve 표기: `OD-12-impl #N → 선택 / 날짜 / owner`. 이 패킷은 *준비*만 — 아래 체크박스를 **채우지 않는다**.

**선결 트랙(Track 0) — OD-12 방향과 무관, 지금 착수 가능**
- [ ] PR-0a 착수 인가 — catalog-gen `ui_primitive`(F4 도입 차단 해소). 출력 포맷 불변.
- [ ] PR-0b 착수 인가 — manifest `source:` 토큰화.
- [ ] PR-0c 착수 인가 — doctor/preflight **warning-only**(exit 0 불변) 확인.

**PR-A 를 열기 위한 최소 묶음**
- [ ] **#8** `layers` 물리 위치 = tier1 **OD-10 과 같은 파일**에서 결정(상속).
- [ ] **#9** fact v1 범위 = **`dir_has_files` 만** 동결.
- [ ] **#5** `edits_at` = ☐ 손실 alias 잔류(§3.1 표기) / ☐ 완전 삭제 — **둘 다 canonical 부활 금지**.
- [ ] merge 규칙 = tier1 `mergeRoles`(layer 단위 교체) 계승 **확인**(재litigate 아님).
- [ ] SoT 경계 = role-token 셀 `layers/access` authoritative **확인**(역전 불가).

**PR-C / PR-E 추가 묶음** (PR-A 뒤 병렬)
- [ ] **#6** 게이트 강도 = 신규 계층 `access`-only 기본; expo `hook` 에 `gates` 붙일지(붙이면 현 requires 와 동형) 결정.
- [ ] **lint-DAG** = import-경계 subset(`route_entry` 제외) + `forbid_upward` + **warning-first** 확인.

**PR-D(★load-bearing)를 열기 위한 추가 묶음**
- [ ] **#4** 정책 파일 전환 = ☐ (a) 즉시 생성물화 / ☐ (b) 점진 전환. 보수적 기본 (a).
...
|---|---|---|---|---|
| #4 정책 전환 방식 & 머지 | I1 | source of truth 경계(*운영*) | PR-D | 열림(a/b 택1) |
| #5 `edits_at` 잔류/삭제 | I2 | edits_at 삭제/alias 잔류 | PR-A | 열림 |
| #6 게이트 강도 | I3 | (게이트 의미) | PR-C/PR-D | 기본 access-only |
| #7 byte-동치 범위 | I4 | access matrix byte-동치 | PR-B→PR-D | 2면 확정, 테스트로 강제 |
| #8 `layers` 물리 위치 | I5 | layers 위치 | PR-A | OD-10 동봉 |
| #9 fact 종류 | I6 | fact v1 범위 | PR-A/PR-C | v1=dir_has_files |
...
| SoT 경계(*정의*) | — | source of truth 경계 | (확인) | §5 확정 |
| merge 규칙 | — | merge 규칙 | (확인) | mergeRoles 계승 |
