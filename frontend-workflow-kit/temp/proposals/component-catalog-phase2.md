...
### Recommendation — **(6b) 우선: 새 메타데이터는 기존 `## Components` 테이블을 깨지 않는 *추가 섹션/추가 컬럼은 별도 섹션으로* 도입한다.** 컬럼을 기존 테이블에 직접 끼워넣는 (6a) 는 기존 모든 행 byte 를 바꾸므로 **회귀-위험이 가장 크다 → 비권장.** 별도 산출물 (6c) 는 가드 표면을 2배로 늘리므로 멀티-tier(OD-4) 같은 명확히 분리된 경우로 한정.
...
2. **(FUTURE) PR-3 — phase2-1 첫 additive 컬럼/섹션:** OD-1 을 닫아 가장 검증 가능한 후보(3a default-export 수집 유력)를 §6(6b) additive 섹션/컬럼으로 도입 + **새 골든 픽스처**. 두-run 결정성·byte-exact 입증(§9). 무타임스탬프·plain sort 유지.
