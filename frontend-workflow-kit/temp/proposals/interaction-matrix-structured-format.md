...
`deriveMetrics` 의 `interaction_matrix_complete` 는 v2 모드에서 "Result 또는 (Result Type+Target) 중 하나라도 채워졌는가"로 확장 가능하나(§2.3), **이 fact 는 여전히 게이트가 아니다** (§1.4). 정확히: 이 fact 는 workflow-state 가 방출하고 readiness 가 fact 로 받지만(`scripts/readiness.mjs:42,179`), 어떤 정책 `requires` 도 그것을 쓰지 않는다(`roadmap-current.md:54`). 따라서 dual-read 의 completeness 변화는 **현 정책 하에서 `readiness_mode` 를 바꾸지 않는다** — fact 자체는 계속 방출·보고될 수 있다(readiness 를 "지나가되" 모드를 막지 않는다). 확장 자체가 **OD-5** 로 열려 있고 v2 PR 의 필수가 아니다.
