...
PR #71 스파이크를 **독립 재현**했다(`node temp/runs/axis2-a-blocker1-spike/synth-policy.mjs`):
...
layers:
  order: [route_entry, screen, domain_component, hook, api_client]   # 화면 근처 → 데이터 근처
access:                              # layer → 편집 허용 모드(allow) / 명시 금지 모드(forbid)
  route_entry:      { allow: [route-skeleton] }
  screen:           { allow: [screen-skeleton, rough-fixture-ui, final-fixture-ui],
                      forbid: [api-integrated-ui] }                  # fake-hook 재-잠금
  domain_component: { allow: [rough-fixture-ui, final-fixture-ui] }
  hook:             { allow: [rough-fixture-ui, api-integrated-ui] } # final 에서 GAP(비연속) — 의도
  api_client:       { allow: [api-integrated-ui],
...
- **머지 규칙:** tier1 의 `mergeRoles`(L70-76) 우선순위를 계승 — `preset.layers < project-layout.layers <
  ...
  **∪** 손수 리터럴 셀.
- **drift 방지 = 기존 `_meta` 멱등성 CI**(재생성→git diff→exit 1, `CI yml:47-52`)를 그대로 사용 — 커밋된
  resolved 정책이 `synthesize(layers)` 와 다르면 실패. 이미 생성물에 쓰는 동일 메커니즘이라 새 게이트 아님.
...
**서로 다른 관계지만 같은 `layers.order` 를 척추로 공유 — 한 선언의 두 투영(projection).** 합치지 말 것:
...
| 출처 | §3 `access:` 행 | §6 `layer_rules`(`order` + `forbid_upward` + `allow:` 예외) |

- 둘 다 ordered `layers:` 의 투영: `order:` 가 척추, lint DAG 는 `forbid_upward`+명시 `allow:` 엣지(FSD
  6레벨, Clean Arch 의존성 역전 screen→use_case→repository-interface)를 더하고, access 는 per-mode allow/forbid
  행을 더한다. **한 `layers:` 선언, 두 파생 산출물**(시간적 edit-gate + 정적 import-DAG). 한쪽 셀이 다른 쪽
  셀을 함의하지 않으므로 한 표로 합치면 안 된다.
- 함의: §6 `layer_rules` 는 §3 access 의 **형제**(같은 `order` 공유)이지 "또 다른 축"이 아니다 → "일반화이지
  새 축 아님" 프레이밍과 정합.
...
- [ ] 정책 role-token 셀이 *생성* 으로 바뀌면 **멱등성 CI**(`_meta` diff) 대상에 포함되는지 확인.
...
>
> PR #71 스파이크를 독립 재현했다(`node temp/runs/axis2-a-blocker1-spike/synth-policy.mjs`):
...
>    → 멱등성 CI 로 drift 차단. §5/§11 충돌은 "role-token 셀 vs 리터럴/requires" 경계로 분리하면 둘 다 성립.
