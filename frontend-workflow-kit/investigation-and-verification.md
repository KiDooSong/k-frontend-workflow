...
새 입력이 들어왔는데 바로 반영하기 어렵고 검증이 필요하면 Reconciliation은 Investigation을 만든다.

```txt
input result
→ reconcile input
→ "검증 없이는 결정 불가"로 분류
→ investigation / verification 생성
→ readiness는 blocks_mode 이전까지만 허용
```
...
MVP-A에서는 Investigation/Verification을 readiness에 직접 연결하지 않아도 된다. 대신 관련 ScreenSpec의 `Unknowns` 또는 `Open Decisions`가 blocker 역할을 한다.
