...
<!-- Direct Screen Import: forbidden = screen file 이 직접 import 금지(shell 이 렌더) / allowed = 화면 직접 사용 허용.
     Catalog Status 권장 enum — cataloged | missing | domain | out-of-scope:
       cataloged    = component catalog 에 있어야 함 (없으면 검사 4 warning).
       missing      = component-gap-register 제안(G-xxx) 대상.
       domain / out-of-scope = ui_primitive catalog 대상이 아닌 domain/shell-owned component 를
                     명시적으로 기록하는 값 — 검사 4 가 warning 대신 info 로 남긴다 (silent pass 아님).
     Catalog Status 는 참고 표기일 뿐 — 존재 정본은 design/component-catalog.md 이고 검사가 대조한다.
     Applies To Families 는 **콤마로만** 구분한다 (family 이름은 공백을 포함할 수 있다). -->
