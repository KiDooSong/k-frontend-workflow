...
<!--
  ...
  - 전체 계약: ../../docs/reference/input-reconciliation.md  (Input Result Contract / Reconciliation Register).
  - kit repository fixtures may include input examples, but packed consumer payloads do not include examples/.
  ...
  저장 위치:
  - flat(기본): docs/frontend-workflow/inputs/{input_id}.md  (파일명 = input_id — grep 추적 용이)
  - 대규모 레포(권장): docs/frontend-workflow/inputs/{domain}/{input_id}.md
    (workflow:create-input --group-by domain 또는 --input-subdir <path>; 파일명은 항상 {input_id}.md)
  - input_id 는 모든 하위 디렉토리를 통틀어 전역 유일하다. validate 검사 11 은 inputs/** 를 재귀 스캔한다.
  - inputs/README.md·index.md 는 그룹 가이드/인덱스이며 입력 결과물이 아니다(validate 가 건너뛴다).
