...
[NEEDS           "마커 0개" 를 다음 단계 진입 게이트로 쓴다.
 CLARIFICATION]  → 본 킷의 저작 규칙(추측 금지·행 추가·모드에서 멈춤)과 역방향 게이트의 근거.
```
...
`validate.mjs` 는 형식 검사 + **역방향 게이트** 를 수행한다.
...
역방향 게이트 (이 설계의 본래 목적 — LLM 의 무단 결정 탐지):

```txt
fact_mode >= open decision 의 Blocking Mode 이면  →  validate 실패.
(fact_mode = Readiness 가 decision 을 무시하고 계산한 "사실이 만족하는 최고 모드")
```

`implemented_mode` 를 파일에서 **새로 추론하지 않는다.** 그 방식은 (1) allowed_paths 가 겹치고 화면-특정적이지 않아 오탐을 내며(예: `src/api/schemas/*` 는 rough-fixture-ui 에서도 스키마 검증용으로 존재 가능), (2) `rough→final` 승격은 *같은 경로*를 편집하므로 어떤 파일 증거로도 구분 불가, (3) 별도 추론기는 "모드 판정 로직 한 곳" 불변식을 깬다. 대신 **Readiness 가 이미 계산한 `fact_mode`(사실 기반)** 를 재사용한다 — fact_mode 가 어떤 open decision 의 Blocking Mode 에 도달했다는 건 "사실이 그 decision 이 막아야 할 지점을 이미 넘었다"는 뜻이다.

경로를 넘는 케이스(예: api-integrated 가 막혔는데 `src/api/**` 를 건드림)는 정책의 `forbidden_paths` 를 재사용해 보완한다. 두 검사 모두 `implementation-mode-policy.yaml` 한 곳의 정의를 재사용하므로 새 추론 surface 가 없다. hook 없는 환경(CI)용 backstop 이라 초기엔 **경고로 시작**해 점진 강화한다.
...
6. `validate.mjs` 가 표 형식 + 전역 ID 중복 + **역방향 게이트**(경고로 시작)를 검사한다.
...
- **LLM 이 게이트를 셀프 개방할 수 없다** — open 은 올리기만 가능하고, 해제는 사람만. 무단 진행은 역방향 게이트가 잡는다.
