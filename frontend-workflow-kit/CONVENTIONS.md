...
Slice Paths are canonical exact paths or terminal `/**` subtrees only; `.`/`..`
segments and arbitrary globs are invalid. Before editing a concrete path, run
`workflow:readiness -- --screen <ID> --path <path> --json`;
