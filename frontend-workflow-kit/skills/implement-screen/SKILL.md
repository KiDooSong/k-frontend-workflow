...
- task-artifact matrix (`docs/reference/task-artifact-matrix.md`) when deciding secondary artifact updates
...
Check `docs/reference/task-artifact-matrix.md` for secondary artifact updates before final report:
- if an approved shared/common component changed under `roles.ui_primitive`, run `npm run workflow:catalog`;
- if route or navigation sources changed, run `npm run workflow:route-tree`, `npm run workflow:nav-graph`, and applicable route cross-checks;
- if generated files may be stale, run advisory `npm run workflow:check-generated` or the specific regeneration command from `docs/reference/generated-files.md`.
...
- validation 결과
- task-artifact matrix 확인 결과와 재생성한 generated files
