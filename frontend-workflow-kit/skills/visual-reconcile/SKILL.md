...
4. visual consistency contract 가 없거나 너무 빈약하면, 또는 multi-screen visual 입력의
   candidate family 를 아직 모르면 [visual-contract-bootstrap](../visual-contract-bootstrap/SKILL.md)을
   preflight 로 먼저 쓸 수 있다 (`workflow:visual-contract-bootstrap --json`). 킷 도입
   초기라 repo 전체 진단이 같이 필요하면 `workflow:adoption-probe -- --visual` 의
   visual observation(probe run dir 의 bootstrap draft + findings)을 본 뒤 진행해도 된다.
   어느 쪽이든 출력은 **review-only** 다 — 자동 적용하지 말고, 사람이 승인한 rows 만
   아래 Update rules 경로로 canonical contract 에 반영한다.
...
[템플릿](../../templates/design/visual-consistency-contract.template.md)으로 draft 제안하거나
[visual-contract-bootstrap](../visual-contract-bootstrap/SKILL.md)으로 후보 초안을 뽑을 수 있다),
