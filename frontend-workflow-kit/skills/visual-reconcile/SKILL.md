...
4. visual consistency contract 가 없거나 너무 빈약하면, 또는 multi-screen visual 입력의
   ...
   preflight 로 먼저 쓸 수 있다 (`workflow:visual-contract-bootstrap --json`). bootstrap
   출력은 **review-only** 다 — 자동 적용하지 말고, 사람이 승인한 rows 만 아래 Update rules
   경로로 canonical contract 에 반영한다.
