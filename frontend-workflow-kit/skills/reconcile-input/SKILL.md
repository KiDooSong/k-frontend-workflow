...
2. Register에서 같은 `input_id` 행을 확인한다:
   ...
   - `failed` → 새 행을 만들지 않는다. 같은 행을 `in-progress` 로 재개하고, 이전 실패 사유를 `Result` 에 보존하거나 retry note 를 이어 붙인다.
...
|---|---|---|---|---|---|---|---|
```
- `Reconcile Status`: `not-started` → `in-progress` → `reconciled` / `failed`. Row 없음과 `not-started` 는 기본 경고(`--enforce` 에서 에러), `in-progress`/`failed`/enum/중복/컬럼 누락은 항상 에러.
