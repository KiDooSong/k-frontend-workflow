---
name: e2e-agent
description: frontend-workflow ScreenSpec/visual 계약에서 Playwright 웹 E2E evidence를 계획, 생성, 검증, 수리하거나 Expo Web visual capture를 기록한다. 사용자가 "e2e 짜줘", "웹 검증", "플레이라이트 테스트", "Playwright", "web evidence", "test plan", "run e2e", "heal failing test", "screenshot", "visual capture", "Expo Web visual evidence", "Figma alignment screenshot"을 요청할 때 사용. 선택형 증거이며 readiness, CI, 승인 게이트가 아니다.
---
...
- 시각 evidence가 필요하면 [figma-component-mapping.template.md](../../templates/screen/figma-component-mapping.template.md) 계열 문서. cross-screen visual ownership 맥락(visual consistency contract)은 [visual-reconciliation.md](../../docs/reference/visual-reconciliation.md)가 정본 — capture candidate가 contract를 참조해도 capture는 advisory evidence로 남는다.
...
Visual capture artifact는 run output이며 기본 커밋하지 않는다:
...
| `capture` | canonical `screen_id`, route or journey setup path, runnable Expo Web surface, known Playwright project/viewport, scoped capture root/locator, `entry_context` classification, `E2E_RUN_ID` |
