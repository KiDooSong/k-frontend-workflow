...
> MVP-B lint-pack 추가분: `scripts/lint-gen.mjs`(PR-2 deterministic flat-config fragment emission) · `skills/adapt-lint-pack`(PR-3 brownfield proposal workflow) · `scripts/lint-baseline.mjs`(PR-4 warning-first ratchet comparison; `--enforce` 때만 증가 exit 1). CI/hard gate 승격은 하지 않는다.
...
| [docs/workflows/lint-policy-catalog.md](docs/workflows/lint-policy-catalog.md) / [lint-policy-rollout-ratchet.md](docs/workflows/lint-policy-rollout-ratchet.md) | MVP-B lint-pack policy catalog · rollout/adoption 계약 | **MVP-B PR-1~4** | 📄 PR-1 schema/template/docs + 🆕 PR-2 `lint-gen.mjs` deterministic `eslint.workflow.config.mjs` emission(`--check`) + 🆕 PR-3 `skills/adapt-lint-pack` scan/propose workflow + 🆕 PR-4 `lint-baseline.mjs` warning-first ratchet runner — CI/gate promotion 후속 |
...
# MVP-B lint-pack PR-2 생성기 (deterministic eslint flat-config fragment — CI/게이트 미승격):
...
# MVP-B lint-pack PR-4 ratchet runner (warning-first; CI/게이트 미승격):
