...
| [docs/workflows/lint-policy-catalog.md](docs/workflows/lint-policy-catalog.md) / [lint-policy-rollout-ratchet.md](docs/workflows/lint-policy-rollout-ratchet.md) | MVP-B lint-pack policy catalog · rollout/adoption 계약 | **MVP-B PR-1~5** | 📄 PR-1 schema/template/docs + 🆕 PR-2 `lint-gen.mjs` deterministic `eslint.workflow.config.mjs` emission(`--check`) + 🆕 PR-3 `skills/adapt-lint-pack` scan/propose workflow + 🆕 PR-4 `lint-baseline.mjs` warning-first ratchet runner + 🆕 PR-5 warning-first CI 관측 — hard gate/required check 는 후속 Open Decision 전까지 미승격 |
