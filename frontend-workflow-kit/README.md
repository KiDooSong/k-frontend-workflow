...
> 현재 단계: **MVP-A** 강제 코어 + **MVP-B Phase 0**(golden fixture 회귀 하니스 · forbidden_paths backstop[warning-first] · 입력/register 검증 검사 11·12) + **MVP-C Phase 1 생성 뷰**(route-tree·nav-graph·component-catalog, 읽기 전용). 추가 랜딩: 검사 8 API-스키마 매칭(엔드포인트 단위·하드) · Interaction Matrix v2 dual-read(warning-first) · Tier2 route-tree 어댑터 솔기 · **Tier2 codegen 어댑터 첫 slice + real emitter slice**(`codegen-core` + `openapi-client` 후보 manifest golden + 실제 client/hook TS golden, hard gate 없음) · Follow-up Quarantine/Role Expansion 설계 초안(#50, docs-only, 게이트 0) · **lint-pack PR-2 lint-gen**(deterministic `eslint.workflow.config.mjs` 생성기 + `--check`) · **lint-pack PR-3 adapt-lint-pack**(brownfield scan/propose 스킬, drafts/reports only, 승인 전 생성기 실행 없음) · **lint-pack PR-4 lint-baseline**(warning-first ratchet runner, `--enforce` opt-in) · **lint-pack PR-5 smoke CI**(`lint-gen --check` + `lint-baseline --json` warning-only 배선). evidence 기반 gate promotion decision·hard gate·Figma·generated-file guard·custom codegen adapter dogfood·OD-5/OD-6/OD-7 확정은 이후 잔여 — 상세·티어링: [roadmap-current.md](roadmap-current.md). ([docs/workflows/mvp-b.md](docs/workflows/mvp-b.md))
