...
킷의 핵심 루프는 다음이다([roadmap-current.md](../../../kit-dev/roadmap-current.md)):
...
다른 모든 축엔 기계 검사가 있다 — validate 12종, readiness, fixture 회귀, 검사 8(API), 검사 13(route). **시각엔 자동 검증이 전무하다.** [roadmap-current.md](../../../kit-dev/roadmap-current.md) 게이트 인벤토리의 fact 어디에도 "스크린샷 일치율·토큰 일치·간격 검사"가 없다.
