...
- 2026-07-11 issue #163 (IMP-02) landed a second opt-in, `workflow:doc-drift --include release-consistency`: narrow structural release/version ↔ implemented-status contradiction rules (§6 "Phase 2" 계열의 좁은 변형) — package version ↔ latest CHANGELOG release heading, roadmap 스냅샷 date ↔ latest release date (age rule only via explicit `--now`, no wall clock), existing package script ↔ same-line 미구현 wording naming the exact script token, and fixed doc counts (`검사 N종` vs the validate.mjs success line — warning; `스크립트/CLI N개` vs top-level `scripts/*.mjs` — info-only). Scope item "manifest active/planned ↔ roadmap" stays with the existing status-heuristic; the includes compose. Scanning is limited to a canonical doc allowlist with mechanical HISTORICAL-marker exclusion (historical/archive docs never flagged), every finding carries `canonical_owner` + `fix_path`, the detector never edits documents, and exit stays 0 (warning-first, no gate promotion). Default Phase 0 output remains byte-identical; telemetry does not forward this include.
