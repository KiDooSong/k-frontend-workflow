...
- 2026-07-06 (follow-up) implemented the opt-in adoption ingest surface: `workflow:telemetry --include adoption --adoption-run <dir>` (or `--adoption-summary <file>`) normalizes an EXISTING `workflow:adoption-probe` run's `probe-summary.json` into one `adoption-probe-summary` surface (group `adoption`, kind `ingest`, determinism witness `normalized-summary-json`). Telemetry never runs adoption-probe, never creates probe run dirs/drafts, and never parses raw observation files; `--include all` never implies adoption. Missing/invalid summary = `available:false` + exit 0; only CLI usage errors exit 2. Default surface set, default ledger byte shape, and the CI artifact posture are unchanged — visual/adoption telemetry in CI artifacts stays a separate human Open Decision, and so does letting telemetry run adoption-probe itself.
