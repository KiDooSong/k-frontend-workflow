...
> 대상 앱: [`temp/runs/figma-fidelity-001/app`](../../../temp/runs/figma-fidelity-001/app) (Expo SDK 56 · react-native-web · NativeWind) — figma 세션이 만든 데모 앱.
> 하니스: [`temp/runs/maestro-dogfood-001/`](../../../temp/runs/maestro-dogfood-001) (앱과 격리). 원시 로그: [run-report.md](../../../temp/runs/maestro-dogfood-001/run-report.md).
> 스코프: **L010 회원가입 랜딩** 1화면 + 그 분기 · **Web 표면만**(네이티브=Maestro, 범위 밖).
...
- **대상/표면**: L010 회원가입 화면(홈 → "로그인" → L010 → provider 선택 → 결과/J020). Web(react-native-web)만.
...
- **두 실험**:
  - **A (baseline)** [`tests/web/L010.spec.ts`](../../../temp/runs/maestro-dogfood-001/tests/web/L010.spec.ts) — `getByRole` + 접근성 이름(앱 무수정).
  - **B (testID)** [`tests/web/L010.testid.spec.ts`](../../../temp/runs/maestro-dogfood-001/tests/web/L010.testid.spec.ts) — 컴포넌트에 `testID` 박고([app-testid.patch](../../../temp/runs/maestro-dogfood-001/app-testid.patch)) `getByTestId` 로 전환. 실험 후 **앱은 원복**, 변경은 패치로만 보존.
...
| 실험 | 선택자 | 결과 | 핵심 |
|---|---|---|---|
| **A** baseline | `getByRole('button',{name})` + `getByText` | **9/9** | 앱 무수정으로 즉시 가능. 단 선택자가 **카피에 결합**(`'Google로 가입하기'`) |
| **B** testID | `getByTestId('signup-provider-google')` | **9/9** | 선택자가 **카피에서 분리**. testID 추가는 role 테스트도 안 깸(하위호환) |
| **드리프트** F9 | — | **라이브 포착** | screen-spec "email→결과 'email'" 인데 구현은 "email→J020 폼"으로 이동 |

**F9가 이 도그푸드의 백미다.** 처음 9/9 green이던 email 테스트가 재실행에서 실패 → 부하/플레이키 의심했으나 워커 1개로도 일관 실패 → 라이브로 클릭해 보니 **다른 세션이 앱에 J020 이메일 가입 플로우를 추가하고 `email` 라우팅을 바꿔** 둔 것. 즉 **E2E가 "문서 계약 vs 실제 구현"의 어긋남을 잡아낸 것**이고, 이건 Healer가 "그냥 green 만들기"로 덮으면 안 되는(=overfit) 정확한 사례다. 테스트는 현실(email→J020)에 맞춰 고치되, **stale해진 screen-spec 행은 reconcile 필요**로 보고에 남겼다(조용한 green 금지).
...
2. **킷 포맷 screen-spec = turnkey 입력.** [L010 screen-spec](../../../temp/runs/figma-fidelity-001/screen/L010-login-main.screen-spec.md)의 State/Interaction Matrix가 플랜 시나리오로 1:1. "spec=진실원천, test=evidence" 규율이 마찰 없이 성립(D-L010-1 시각 OD는 시나리오 0개 제거, U-L010-2 open은 로그인 링크를 데모값까지만 단언).
...
6. **라이브 검증이 selector 함정을 잡음(F4).** "회원가입"은 heading이 아니라 plain `<Text>` → `getByRole('heading')` 대신 `getByText`. 01 §c.2 경고를 실제로 만남 = Generator의 "직렬화 전 라이브 검증"의 가치.
...
1. **testID를 "스펙 선언 계약"으로 승격.** 누가/언제/어떻게 = **screen-spec 저자**가 안정 앵커를 Accessibility/Interaction 계약에 선언 → **구현자가 screen-skeleton~fixture-ui 단계에 코드에 삽입**(Generator는 *소비*만). 네이밍 규칙(`{screen}-{element}`, 리스트 `{entity}-item-${id}`)은 **llm-rules에 고정**. → 현재 킷엔 testID 규칙이 0개라(골든 예제·템플릿·llm-rules 전수 확인) 이건 *처방됐으나 미채택*인 갭. 이 도그푸드가 그 갭을 실증.
...
5. **하니스는 per-run 격리 유지.** 앱과 분리 + `reuseExistingServer` + 실험 후 앱 원복(변경은 패치로). 다중 세션 동시 작업 환경에서 안전.
...
npx playwright test tests/web/L010.testid.spec.ts   # 실험 B(testID) — 9/9
```
> 주의: `app-testid.patch` 는 figma 앱(`../figma-fidelity-001/app`)에 적용. 본 도그푸드는 적용→검증 후 **앱을 원복**했으므로, 실험 B 재현 시에만 패치를 다시 얹는다.
