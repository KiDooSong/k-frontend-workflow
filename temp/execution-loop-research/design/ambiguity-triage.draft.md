...
1. **후보만 제안.** LLM 은 U-cand / D-cand 를 *제안*한다. 닫거나, ScreenSpec 의 `## Unknowns` / `## Open Decisions` 에 직접 써넣거나, `candidate→confirmed` 로 올리거나, Conflict 를 close 하지 않는다(불변식 4 · SYNTHESIS §9.6 마지막 항).
...
각 애매함에 대해 아래 신호를 평가한다. **하나라도 "강"이면 Open Decision 후보(D-cand)로 제안**한다(나머지가 약해도). 전부 "약"이면 Unknown(U-cand)으로 남긴다. "강 신호의 blocking" 은 `reports/01` §3 DoR 해소와 정합 — **warning 텍스트로 표면화하되 실제 cap 은 기존 readiness 가 건다**(신설 게이트 아님).
...
- LLM 이 `D-cand` / `U-cand` 를 ScreenSpec 의 `## Open Decisions` / `## Unknowns` 에 **직접 확정/close** (승격·resolve·close 전부 사람 — 불변식 4).
