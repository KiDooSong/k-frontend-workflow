...
# Ambiguity Triage Rubric (초안) — Unknown→Open Decision triage

PR1b 의 work-packet 템플릿이 **링크로 참조할 별도 doc**. 최종 위치: `docs/workflows/ambiguity-triage.md` (사람 확정). 템플릿에는 **최소 표만** 넣고(과적재 방지, SYNTHESIS §9.5), 결정트리·신호표·Blocking Mode 매핑·모드별 Safe-To-Proceed 의 **전체**는 이 doc 에서만 유지한다.

- 정본 출처: `reports/01-ambiguity-gate.md` §4 (Concrete deliverable — rubric).
- 어휘 정본: `SYNTHESIS.md` §9.1. 층위 구분: §9.2. 코드화 금지: §9.6.
- 모드 사다리 정본: `frontend-workflow-kit/policies/implementation-mode-policy.yaml` (`order:`).

> ⚠ 적용 범위 주의: 이 doc 은 **사람이 읽고 LLM 이 대화로 따르는 판단 가이드**다. 어떤 부분도 스크립트/게이트로 코드화하지 않는다(§7, SYNTHESIS §9.6). 유일한 코드 게이트는 변함없이 `readiness.mjs`(Open Decision cap + 정책 fact) + `validate.mjs`(구조)뿐이다.

---

## 1. 머리말 — 이 doc 이 무엇이고 무엇이 아닌가
...
- **무엇이 아닌가**:
  - 게이트가 아니다. 이 doc 의 어떤 분기도 exit 1 / 머지 차단 / required check 로 배선하지 않는다(§7).
  - 결정을 닫지 않는다. 승격(promote)·해소(resolve)·종료(close)·`candidate→confirmed` 는 **전부 사람** 몫이다(공통 불변식 4).
  - 새 산출물 축이 아니다. 분류 결과가 사는 곳은 기존 Work Packet 의 `Ambiguity Review Required` 섹션(후보 표)과 ScreenSpec(사람이 확정한 뒤)뿐이다.
- **링크 관계**: PR1b 템플릿 → 이 doc(전체 rubric). 템플릿은 "New Unknowns / New Open Decision Candidates / Possibly Blocking Ambiguities / Safe To Proceed?" 의 **빈 표 골격 + 이 doc 으로의 링크 한 줄**만 갖는다.

---

## 2. 목적 / 원칙

**목적**: 구현 전에 애매함을 표면화한다. 시스템의 첫 질문은 "구현 가능?"이 아니라 **"애매한 거 놓친 거 없나?"** 이다(`reports/01` §1, SYNTHESIS §1).

**원칙 (전부 동시 성립):**

1. **후보만 제안 (이 triage 를 쓰는 단계 = `workflow:packet` / `workflow:run` / review).** 이 단계의 LLM 은 U-cand / D-cand 를 *제안*만 한다 — 분류 결과를 Work Packet 의 `Ambiguity Review Required` / Review Evidence 에 후보로 올리되, ScreenSpec 의 `## Unknowns` / `## Open Decisions` 에 **자동 반영하지 않는다**.
   - 단, **ScreenSpec authoring / reconcile 단계**의 LLM 은 기존 계약대로 `## Unknowns` / `## Open Decisions` 에 **`open` 행을 추가**하거나, 새 입력이 기존 `resolved` 결정과 충돌하면 `resolved → open` 으로 재오픈할 수 있다(`global/llm-rules.md` · `input-reconciliation.md` — 게이트를 *올리는* 방향만).
   - 어느 단계든 **resolve / close / `candidate→confirmed` 승격 / 재-resolve 는 항상 사람**(불변식 4 · SYNTHESIS §9.6 마지막 항). 즉 "직접 반영하지 않는다"는 이 triage(packet/run/review) 단계 한정이며, "LLM 이 ScreenSpec 에 아무것도 못 쓴다"는 뜻이 아니다.
2. **calibrate, 무차별 질문 금지.** 모든 불확실성을 D-cand 로 올리지 않는다. **강(strong) 신호**가 켜질 때만 Open Decision 후보로 제안하고, 약한 불확실성은 Unknown 으로 남긴다(clarification fatigue·checklist-worship 회피 — `reports/01` §2.1·§6, *Ask or Assume?* / ClarifyGPT 의 calibrate 교훈).
3. **전부 warning-only.** 이 doc 의 분류·`Safe To Proceed?` 는 readiness/validate 같은 **공식 게이트로 승격하지 않는다**(warning-first 층위 — SYNTHESIS §9.2). exit 1 금지.
4. **유일 게이트 = readiness(Open Decision cap) + validate(구조).** 실제로 도달을 막는 권한은 사람이 ScreenSpec 에 확정한 Open Decision 과 정책 fact 뿐이다.
5. **green ≠ done.** `Ambiguity Review` 가 비었다는 것은 "이번 패스가 새 애매함을 못 찾음"일 뿐 "설계가 충분함"이 아니다. 의미/제품 리뷰(Codex/사람)는 별도다(Track 05 접점).
6. **형식주의 회피.** 빈 블록은 "없음 — <사유>" 한 줄로 명시한다. 억지 행은 신호를 희석한다.

**warning-first 와 auto-stop 은 다른 층위다(충돌 아님 — SYNTHESIS §9.2):**
...
발견한 애매함 하나를 아래 트리에 통과시킨다. 출구는 세 가지: **(a) Open Decision Candidate 로 승격 제안**, **(b) 그냥 Unknown(=U-cand) 유지**, **(c) Possibly Blocking 으로 표시(`Safe To Proceed?`=no 후보, 사람 판단 요청)**. 어느 출구든 닫는 건 사람이다.
...
- **(a) D-cand**: Work Packet 의 `New Open Decision Candidates` 표에 후보로 적는다. Blocking Mode 는 §5 표로 **제안값**만. 승격(= 사람이 ScreenSpec `## Open Decisions` 에 open 행 확정 → `npm run workflow:readiness` 재실행)은 사람.
- **(b) U-cand**: Work Packet 의 `New Unknowns` 표에 후보로 적고, "ScreenSpec `## Unknowns` 에 open 행 추가"를 *제안*만 한다. Unknown 은 기본 게이트가 아니다(불변식 5) — 막아야 할 애매함이면 (a) 또는 (c)로 간다.
- **(c) Possibly Blocking**: `Possibly Blocking Ambiguities` 표에 켜진 신호와 함께 "사람이 판단해 달라"로 남긴다. 판단 전까지는 fail-safe — 해당 모드를 `Safe To Proceed?`=no 쪽으로 보수적으로 반영한다(`reports/01` §6 권고: MVP=fail-safe, malformed→docs-only fail-closed 정신과 일치).
...
각 애매함에 대해 아래 신호를 평가한다. **하나라도 "강"이면 Open Decision 후보(D-cand)로 제안**한다(나머지가 약해도). 전부 "약"이면 Unknown(U-cand)으로 남긴다. ⚠ **"강 1개 이상 → D-cand"는 초기 *보수* 기본값**이다(over-asking 위험을 감수하고 안전 쪽으로) — 실제 임계는 coupon-feature A/B 측정으로 보정한다(§8.1, SYNTHESIS §8-4). "강 신호의 blocking" 은 `reports/01` §3 DoR 해소와 정합 — **warning 텍스트로 표면화하되 실제 cap 은 기존 readiness 가 건다**(신설 게이트 아님).
...
**신호 강도 → 권고 route (review/triage 산출물의 route 필드와 동일 어휘 — SYNTHESIS §9.3):**
...
> route 어휘 주의: `recommended-fix | human-only-decision | do-not-auto-fix` 는 review-artifact advisory 스키마(SYNTHESIS §9.3)의 `route` 와 같은 값이다. 새 이름을 만들지 않는다. `route` 는 "어떻게 다룰지 권고"일 뿐 — 어떤 값도 자동 차단을 의미하지 않는다.
...
Blocking Mode = "이 결정이 없으면 **도달을 막는** 모드". 그 결정을 cap 하면, readiness 는 `min(fact_mode, decision_cap)` 으로 그 **바로 아래**까지만 연다(`reports/01` §4-2). LLM 은 이 매핑으로 **후보값을 제안**하고, 사람이 ScreenSpec 에 확정한다.

| 결정/애매함이 바꾸는 것 | 제안 Blocking Mode | 비고 |
|---|---|---|
| 문서/라우트 수준만 | (cap 없음) | `docs-only`·`route-skeleton` 통과. 애매함을 문서로 표면화하는 게 목적. |
...
**유형별 cap 보유 규칙 (work-packet 템플릿 Blocking Items 주석과 동일 — verbatim 정합):**
...
- **missing-fact** → cap 모드 보유(예: `component_catalog_generated == false` 면 `rough-fixture-ui` cap). 단 이 doc 의 주 대상은 사람의 선택이 필요한 ambiguity 이고, missing-fact 는 readiness 가 fact 로 직접 본다 — 여기서는 "어떤 fact 가 빠졌는지"만 후보로 적고 cap 은 readiness 출력에서 복사한다.
- **unknown (U-cand)** → 모드를 직접 cap 하지 않으면 `—`. (cap 하려면 (a)/(c)로 승격해야 한다 — 불변식 5: Unknown 은 기본 게이트 아님.)
...
## 6. 모드별 Safe-To-Proceed 표 — auto-stop 판단 (readiness 재계산 아님)

`Safe To Proceed?` 는 Work Packet 의 `Ambiguity Review Required` 하위 표다(어휘 §3). 각 모드에 대해 **"지금 코딩해도 미해결 애매함이 이 모드 산출물을 위태롭게 하나?"** 만 본다. readiness 를 다시 유도하지 않는다.

**사용법(3줄):**
- **아래에서 위로** 훑어 "no"가 처음 나오는 모드 **직전**에서 멈춘다(= `HALT_AMBIGUITY`).
- **천장 불변**: `Safe? = yes` 라도 `requested_mode > readiness_mode` 면 코딩하지 않는다. 천장은 항상 `readiness_mode`(hard ceiling — readiness 가 단일 출처, 불변식 1).
...
## 7. "절대 코드화 금지" 재확인 (SYNTHESIS §9.6 — 이 doc 관련 항)
...
- 강 신호 분류를 **머지 차단 / required-approval(merge check)** 에 배선 (사내 GitLab: MR approval rules·merge checks·protected branches 에 연결 금지 — SYNTHESIS §9.3).
- **blocking Unknown 을 게이트로** 취급 (막는 건 승격된 Open Decision 만 — 불변식 5).
- 이 doc 의 결정트리/신호표를 **자동 분류 스크립트**로 굳혀 readiness 를 덮어쓰기/우회.
...
→ 실제 게이트는 변함없이 **readiness(Open Decision cap) + validate(구조)** 뿐이다. 이 doc 은 그 게이트에 *입력될 후보*를 사람에게 제안하는 대화 가이드일 뿐이다.

---

## 8. 남은 사람-결정 (닫지 말 것 — "결정 필요"로 — SYNTHESIS §8)

이 doc 으로 닫히지 않는다. 사람이 정한다:

1. **calibration 임계의 실전값**: "강 신호"를 몇 개·어떤 조합에서 D-cand 로 승격할지. over-asking 회피 튜닝은 경험적 — 초기 기본값은 "강 신호 1개 이상"(§4)이나, 실측 보정은 별도 트랙(SYNTHESIS §8-4: coupon-feature A/B). **결정 필요.**
...
3. **fail-safe 기본값 확정**: `Possibly Blocking`(c)을 기본 "막음"으로 둘지 repo 차원 합의. MVP 권고는 fail-safe(§3, `reports/01` §6)이나 마찰↑ — **결정 필요.**
4. **이 doc 의 최종 위치/형식**: `docs/workflows/ambiguity-triage.md` 로 확정할지(권고), 아니면 proposal 로 둘지. **결정 필요.**
...
## 부록 A — PR1b 템플릿에 들어갈 "최소 표 + 링크" (과적재 방지)

이 doc 은 전체를 갖고, **PR1b work-packet 템플릿에는 아래 최소 골격만** 넣는다(SYNTHESIS §9.5). 아래는 템플릿용 마크다운 블록 초안이다(삽입 위치/형식은 work-packet PR1b 초안에서 확정):
...
<!--
  ...
  분류 기준(결정트리·신호표·Blocking Mode 매핑·모드별 Safe-To-Proceed) 전체는 별도 doc 참조:
    → docs/workflows/ambiguity-triage.md
...
> 위 블록은 **표 골격 + 링크 한 줄**만이다. 결정트리·신호표·Blocking Mode 매핑·모드별 트리거 설명은 템플릿에 **넣지 않는다** — 전부 이 doc(§3~§6)에서만 유지한다.
