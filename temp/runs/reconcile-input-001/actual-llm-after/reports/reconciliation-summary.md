# Reconciliation Summary — reconcile-input-001
...
|---|---|---|---|---|
| 1 | IN-20260613-planning-001 | simple-update + resolves-decision + scope-unclear | COUPON-001 상태 탭 UI Sections·Interaction·Copy Keys(tbd)·Acceptance 보강, U-002(enum 매칭) open 생성, D-001 비고에 separate-tab 제안 기록 | D-001 을 separate tab 으로 resolve |
...
| 3 | IN-20260613-api-001 | simple-update + resolves-unknown + resolves-decision | api-manifest /coupons → page envelope, U-001 resolved, D-003 옵션 offset/page 로 좁힘, COUPON-001 Data/API Candidates 보강 | D-003 resolve |
...
- **신호만(게이트 아님)**: C-001(conflict, passive log), G-001(component gap 제안), U-002(unknown).

## 불변식 점검
- register-first: 5개 입력 모두 reconciliation-register 에 행 존재, Reconcile Status=reconciled.
- LLM 은 게이트를 올리기만 함: resolved 결정 재-resolve 0건, conflict close 0건, confirmed 승격 0건, gap accept 0건.
- AUTH-001(confirmed) frontmatter 강등 안 함 — D-204 만 open 으로 재오픈(게이트는 readiness 가 건다).
- U-001 은 resolves-unknown 으로 닫음(Unknown 은 비-게이트). U-002 는 새 open.
