...
## API Candidates
- GET /coupons (confidence: candidate) — 응답이 page envelope `{ items, page, size, hasNext }` 로 변경 (api-manifest; 정렬=만료 임박 순, 서버 결정)
...
| coupon.tab.available | 사용 가능 | tbd |
| coupon.tab.used | 사용 완료 | tbd |
| coupon.tab.expired | 만료 | tbd |
...
| U-002 | 상태 탭 3개(사용 가능/사용 완료/만료)가 서버 쿠폰 상태 enum 과 1:1 로 매칭되는가? | open |
