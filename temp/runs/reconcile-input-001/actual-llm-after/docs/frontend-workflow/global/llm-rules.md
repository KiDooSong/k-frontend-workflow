...
- 충돌을 발견하면 임의 판단하지 말고 conflicts(_meta/conflicts.md)에 기록한다.

## 참조 경로
> 이 fixture 는 md-only 샘플이라 conflicts.md 와 decision-log.md 가 `global/` 이 아니라 `_meta/` 아래 있다.
> API 도 확정분(openapi.yaml)은 아직 없고 미확정분(api/api-manifest.md)만 있다.

- 화면 계약: docs/frontend-workflow/domains/{domain}/screens/{screen}/screen-spec.md
- 컴포넌트 카탈로그: docs/frontend-workflow/design/component-catalog.snapshot.md (SAMPLE 스냅샷 — 생성기는 MVP-C)
- API: docs/frontend-workflow/api/api-manifest.md (미확정분; 확정분 openapi.yaml 은 이 샘플에 없음)
...
- 충돌 기록: docs/frontend-workflow/_meta/conflicts.md
- 결정 로그: docs/frontend-workflow/_meta/decision-log.md
...
- 이 fixture 는 코드가 없으므로 사실 기준 readiness 천장은 screen-skeleton 이다. 그 이상(rough-fixture-ui 등)은 fake hook·생성 카탈로그·사람 승인이 더해진 별도 implement-screen 세션에서만 도달한다.
