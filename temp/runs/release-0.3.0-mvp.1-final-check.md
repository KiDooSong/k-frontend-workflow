...
| 4 | 태그 생성 후 release note 와 source commit 일치 | ✅ (2026-07-11 갱신) 태그 `v0.3.0-mvp.1` = PR B 병합 커밋 `fea0501` 에 생성·push 완료 — CHANGELOG `0.3.0-mvp.1` 섹션이 release note |

**판정: GO** — 태그/push 완료(2026-07-11 갱신): tracker #167 대로 PR B 가 통과한 병합 커밋 `fea0501` 에 사람이 태그를 생성했다.

## 8. exact tag command (기록용 — 실행 완료됨)

> ✅ (2026-07-11 갱신) 아래 명령은 이미 실행되어 태그 `v0.3.0-mvp.1` 이 `fea0501` 에 존재한다.
> **다시 실행하지 말 것** — 당시 실행한 명령의 기록으로만 보존한다.
