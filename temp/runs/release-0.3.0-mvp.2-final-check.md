...
warning-first finding 분류(자동 NO-GO 아님): doctor warnings 7건(role-glob 4 + layer-glob 3 —
consumer 미생성 레이어 관측, expected) · doc-drift `broken-relative-link` 2건 — planner 의
`_upgrade/upgrade-plan-<ref>.md` 가 embed 한 upgrade-notes 상대 링크가 plan 파일 위치 기준으로
깨지는 kit 후속 이슈 후보(관측 전용, 릴리스 비차단).
