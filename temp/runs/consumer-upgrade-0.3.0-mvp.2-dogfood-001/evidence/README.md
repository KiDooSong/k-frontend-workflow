...
- [upgrade-plan-dry-run.md](upgrade-plan-dry-run.md) 는 planner 가 생성한 plan 문서를
  그대로 복사한 것이다. plan 이 embed 하는 upgrade-notes 본문의 상대 링크
  (`input-reconciliation.md` · `screen-identity.md`)는 원래 킷의
  `docs/reference/` 기준 링크라서 **이 위치(및 consumer 의 `_upgrade/` 위치)에서는
