...
> 🗄 **HISTORICAL run evidence** — 이 문서는 `v0.3.0-mvp.2` release candidate 시점(2026-07-12)의
> dogfood 실행 기록이다. 인덱스: [../README.md](../README.md).
...
plan: [evidence/upgrade-plan-dry-run.md.txt](evidence/upgrade-plan-dry-run.md.txt) (verbatim 캡처 — `.txt`
확장자는 [evidence/README.md](evidence/README.md) 참조) · [evidence/upgrade-plan-dry-run.json](evidence/upgrade-plan-dry-run.json)
...
- **재현 raw witness**: 같은 pre-upgrade 커밋(`878fd33`)의 detached 임시 worktree 에서 보수적 apply 를
  재실행해 위 주장들을 raw 출력으로 캡처 —
  [evidence/apply-witness-reproduced.txt](evidence/apply-witness-reproduced.txt)
  (`git status --porcelain` tracked modified **0** · `.incoming` 전수 목록 **29** · apply exit 0).

## 4. 사람 리뷰 → conflict 해소 (take-incoming)

리뷰 근거: consumer 의 vendor commit(`107aa39`) 이후 pre-upgrade HEAD(`878fd33`)까지
`tools/frontend-workflow/**` 에 대한 `git diff --exit-code` 가 **빈 diff (exit 0)** — 이 consumer 는
vendored kit 을 한 번도 로컬 수정하지 않았다. raw 캡처(명령·exit code·consumer 커밋 3개의
`git log`, 업그레이드 커밋 `1970a68` 의 `git show --stat` 포함):
[evidence/consumer-git-witness.txt](evidence/consumer-git-witness.txt).
따라서 29건 conflict 는 전부 "unmanaged baseline 이라 증명 불가" 분류이지 실제 로컬
...
| `workflow:doctor` (신규 alias) | n/a | 0 | warning-only: findings 13 (warnings **7** = role-glob 4 + layer-glob 3, info 6) ([after](evidence/after-doctor.txt)) — §7 분류 |
...
| doctor `layer-glob` warning 3건 (`api_client`/`domain_component`/`hook` layer glob 매치 0) | **expected** — 위와 동일 원인 | role-glob 과 같은 미생성 레이어를 layer 축에서 중복 관측한 것 (합계 warnings 7) |
| doctor info 6건 (`layer-access-readiness-wired` 5 + `policy-draft-ready` 1) | **expected** | 정보성 안내 |
...
- Open Decision resolve / `confirmed` 승격 / conflict register close: **수행 안 함** (PROFILE-001 의
  열린 Open Decision `D-301` 이 before/after 동일하게 cap 유지 — evidence 의 blocking_decisions 참조).
