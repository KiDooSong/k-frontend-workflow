...
- **Mechanism:**
  ...
  (3-c) Linked Schema 의 zod 심볼/OpenAPI path 부재 → "지목 스키마 부재" 에러
  ```
  현행 전역 존재검사(`hasZod||hasOpenApi` 1회)가 endpoint 단위 3-스텝 해소로 바뀐다.
- **파서 변경:** 3종 — ① ScreenSpec 측 `parseApiCandidates` 확장(method+path 추출, **불릿 유지라 파서만 확장**) ② api-manifest `## Endpoints` 파서(신규지만 **`parseTable` 재사용** — `Method`/`Path`/`Linked Schema` col 추출) ③ path 정규화 유틸(method 대소문자·trailing slash·`{id}` 통일; **화면 route `[id]` 는 제외**). 단계 분리 권장: 1차는 OpenAPI path/파일 존재 수준, zod **심볼** 존재까지는 2차(zod 파일 내용 파싱이라 범위↑).
- **마이그레이션:** 문서 마이그레이션은 **S** — api-manifest.md **1개 파일** 에 Linked Schema 컬럼 1개 추가 + 행별 값. **ScreenSpec 전수수정 불필요**(불릿 유지, method+path 가 이미 들어있음). 단 코드(파서 3종)와 카탈로그 배선을 합치면 실질 **M.** (권장) `artifact-manifest.yaml` 에 api-manifest 등록(현재 미등록 — 검사 2 가 manifest 존재·경로를 보증하지 않음).
...
1. **마이그레이션 blast radius(최고 가중치):** C 는 문서 변경이 **api-manifest.md 1개 파일**(Linked Schema 컬럼 1개 추가)로 끝난다. ScreenSpec 은 불릿 형식을 유지하고 이미 method+path 를 담고 있어 **전 screen-spec/템플릿을 건드리지 않는다.** A 는 16파일 전수 불릿→표 전환(L), B 는 전수 frontmatter 추가(M) 또는 opt-in 희석. 이 기준에서 C 가 압도적이다.
...
[후속 세션 — 구현. 권장 순서]
P1. 골든 예제 api-manifest.md 에 Linked Schema 컬럼 추가 + 행 값 채움.        ← 문서(S 규모)
...
핵심: **P1 의 문서 변경이 1파일** 이고 ScreenSpec 전수수정이 없다는 점이 옵션 C 채택의 실익이다. P2–P4 는 코드 변경이지만 `parseTable` 재사용으로 신규 인프라가 적다. zod **심볼** 존재 검증(P4 의 3-c)은 단계 분리해 1차는 파일/OpenAPI 존재 수준으로 시작할 수 있다.
...
```txt
입력:   docs/ 트리에서 수집한 specs[](screen-spec)와 1개의 api-manifest(artifact_type:api-manifest).
...
2) 각 screen-spec 에 대해:
   ...
   각 confirmed 엔드포인트 e:
     ...
     (3-c) 인덱스[k].schema 가 가리키는 심볼/path 부재
            - 1차: src/api/schemas/*.ts 파일 존재(현행 dirHasFiles 수준) 또는 openapi.yaml path 존재
            - 2차(후속): zod export 식별자 스캔으로 심볼 실재 확인
                                          → add(8, "...지목 스키마 부재...")
```
...
[FAIL-2 미연결 / 표기 불일치]
  manifest 행은 있으나 Linked Schema 빈칸,  또는 ScreenSpec /coupons/{id} vs manifest /coupons/{couponId}
  → 정규화 후에도 안 맞으면 "스키마 미연결" 또는 "매칭 실패" (정규화 회귀 가드).
...
OD-2. Linked Schema 검증 강도(1차/2차).  1차=파일/OpenAPI path 존재(현행 수준), 2차=zod export 심볼
      실재 스캔. 2차는 zod 파일 내용 파싱이라 범위↑. 어디까지를 MVP-B 1차로 볼지.

OD-3. manifest 부재 시 정책.  폴백(전역 존재검사) vs 엄격(부재 자체를 fail). 본 문서는 폴백 권장 — 확정 필요.
