...
**필요한 입력 파일.**
- 각 `screen-spec.md` 의 `## API Candidates` (현재 `parseApiCandidates` 는 **confidence·raw 만** 추출 — method/path 파싱은 신규 필요. 매칭 키 규약은 [api-schema-linking-decision.md](./api-schema-linking-decision.md) 가 결정).
...
>
> **deprecated alias** (parser 는 한동안 읽되 발견 시 warning, template/generator 는 출력 안 함): ① `suggested_scope.domains/screens` → `affected_domains`/`affected_screens` 로 대체, ② frontmatter `summary` → body `## Summary` 섹션이 정본. (✅ 이 정본은 commit 69e6baf 에서 `input-reconciliation.md` Input Result Contract·template·reconcile-input SKILL 에 반영됨.)
...
**권장 테스트 fixture.** `examples/input-validation/inputs/` — (a) required 9필드 충족(pass), (b) `input_id` 누락(fail), (c) 두 파일 `input_id` 중복(fail), (d) `supersedes` dangling(fail/경고), (e) required 9필드 충족 + optional(confidence/supersedes/raw_artifacts) 생략 → pass(optional 생략 허용 가드); required 누락 → fail.
...
후보 4   ✅ 정본 결정 완료(2026-06-13) + 문서 반영 완료(commit 69e6baf): fixture/template 기준 flat schema 채택,
         input-reconciliation.md·template·reconcile-input SKILL 정렬. suggested_scope·frontmatter summary 는 deprecated alias.
